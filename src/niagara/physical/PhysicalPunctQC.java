...
/**
 * <code>PhysicalPunctuateOperator</code> implements a punctuate operator for an
 * incoming stream;
 * 
...
public class PhysicalPunctQC extends PhysicalOperator {
	...
	// opInitialize
	public PhysicalPunctQC(Attribute pAttr, String timeAttr,
			String queryString, PunctSpec pSpec) {
		setBlockingSourceStreams(blockingSourceStreams);
	...
	public void opInitialize() {
		...
		System.out.println("PunctQC - len input 0 (db): "
				+ inputTupleSchemas[0].getLength());
		System.out.println("PunctQC - first attr input 0: "
				+ inputTupleSchemas[0].getVariable(0).getName());
		System.out.println("PunctQC - len input 1 (stream punct): "
				+ inputTupleSchemas[1].getLength());
		System.out.println("PunctQC - first attr input 1: "
	...
	throws ShutdownException, InterruptedException, OperatorDoneException {
		...
		if (streamId == 1) {
			...
			/*
			 * long start, end; long ts = getTupleTimestamp(inputTuple, 1);
			 * 
			 * start = ts; end = start + queryGranularity;
			 * 
			 * String startTS = new Timestamp(start*1000).toString(); startTS =
			 * startTS.substring(0, startTS.length()-2);
			 * 
			 * String endTS = new Timestamp(end*1000).toString(); endTS =
			 * endTS.substring(0, endTS.length()-2);
			 * 
			 * IntegerAttr sensorid = (IntegerAttr)inputTuple.getAttribute(5);
			 * //String ctrlMsg =
			 * String.valueOf(start)+"\t"+String.valueOf(end); String ctrlMsg =
			 * startTS + "\t" + endTS +"\t"+sensorid.toASCII();
		...
		if (lastts == Long.MIN_VALUE) {
			...
			if (newts != lastts) {
				if (newts < lastts)
					System.err
							.println("data from db is not ordered *******************");
	 ...
	 */
	/*
	 * private String newQuery(long start, long end) { Calendar calendar =
	 * GregorianCalendar.getInstance();
	 * 
	 * //calendar.setTimeInMillis(start*1000); //String startTS =
	 * calendar.getTime().toString(); //calendar.setTimeInMillis(end*1000);
	 * //String endTS = calendar.getTime().toString();
	 * 
	 * System.err.println("start at: "+start+"   end at: "+end); StringBuffer
	 * query = new StringBuffer("(");
	 * 
	 * int numDays = sSpec.getNumOfDays(); int numMins = sSpec.getNumOfMins();
	 * 
	 * //queryString.replace ("NUMOFDAYS", "interval '1 day'");
	 * //query.append(queryString); String upper, lower; switch
	 * (sSpec.getSimilarityType()) { case AllDays: for (int i = 1; i <= numDays;
	 * i++) { if (i > 1) query.append(" union all ");
	 * 
	 * //query.append(queryString.replace("NUMOFDAYS", " '" +i+" days'"));
	 * 
	 * calendar.setTimeInMillis(start*1000); calendar.roll(Calendar.DAY_OF_YEAR,
	 * -i); //calendar.add(Calendar.MINUTE, -numMins); lower =
	 * calendar.getTime().toString();
	 * 
	 * calendar.setTimeInMillis(end*1000); calendar.roll(Calendar.DAY_OF_YEAR,
	 * -i); //calendar.add(Calendar.MINUTE, numMins); upper =
	 * calendar.getTime().toString();
	 * 
	 * query.append(queryString.replace("NUMOFDAYS", " '"
	 * +i+" days'").replace("TIMEPREDICATE", timePredicate(upper, lower)));
	 * 
	 * } query.append(") order by panetime"); break;
	 * 
	 * case SameDayOfWeek: for (int i = 1; i <= numDays; i++) {
	 * //query.append(queryString.replace("NUMOFDAYS", " '" +i+" days'"));
	 * 
	 * calendar.setTimeInMillis(start*1000); calendar.roll(Calendar.DAY_OF_YEAR,
	 * -i*DAYS_PER_WEEK); //calendar.add(Calendar.MINUTE, -numMins); lower =
	 * calendar.getTime().toString();
	 * 
	 * calendar.setTimeInMillis(end*1000); calendar.roll(Calendar.DAY_OF_YEAR,
	 * -i*DAYS_PER_WEEK); //calendar.add(Calendar.MINUTE, numMins); upper =
	 * calendar.getTime().toString();
	 * 
	 * if (i > 1) query.append(" union all ");
	 * 
	 * query.append(queryString.replace("NUMOFDAYS", " '"
	 * +i*DAYS_PER_WEEK+" days'").replace("TIMEPREDICATE", timePredicate(upper,
	 * lower)));
	 * 
	 * } query.append(") order by panetime"); break;
	 * 
	 * case WeekDays: int i = 1, offset = 1, weekday; while (i <= numDays) {
	 * calendar.setTimeInMillis(start*1000); calendar.roll(Calendar.DAY_OF_YEAR,
	 * -offset); weekday = calendar.get(Calendar.DAY_OF_WEEK); if (weekday ==
	 * Calendar.SUNDAY || weekday == Calendar.SATURDAY) { offset++; continue; }
	 * //calendar.add(Calendar.MINUTE, -numMins); lower =
	 * calendar.getTime().toString();
	 * 
	 * calendar.setTimeInMillis(end*1000); calendar.roll(Calendar.DAY_OF_YEAR,
	 * -offset); if (weekday == Calendar.SUNDAY || weekday == Calendar.SATURDAY)
	 * { offset++; continue; } //calendar.add(Calendar.MINUTE, numMins); upper =
	 * calendar.getTime().toString();
	 * 
	 * if (i > 1) query.append(" union all ");
	 * 
	 * query.append(queryString.replace("NUMOFDAYS", " '"
	 * +offset+" days'").replace("TIMEPREDICATE", timePredicate(upper, lower)));
	 * i++; offset++; } query.append(") order by panetime"); break;
	 * 
	 * default:
	 * System.err.println("unsupported similarity type: "+sSpec.getSimilarityType
	 * ().toString());
	 * 
	 ...
	 */
	public Op opCopy() {
		PhysicalPunctQC another = new PhysicalPunctQC(pAttr, timeAttr,
				queryString, pSpec);
