...
/**
 * A <code> MinNodeMerge </code> element will merge two nodes together using a
...
//			Class intClass = Class.forName("java.lang.Integer");
			...
			aggMethod = aggObject.getClass().getDeclaredMethod(fcn, paramTypes);
			name = null;
	...
	boolean merge(Node lNode, Node rNode, Node resultNode)
		...
		try {
			...
			throw new ShutdownException(
					"Invalid Method Invocation - Illegal Access - on "
							+ aggMethod.getName() + "  " + e1.getMessage());
		} catch (InvocationTargetException e2) {
			if (aggObject == null) {
				System.out.println("aggObject is null");
			} else {
				System.out.println("aggObject is "
						+ aggObject.getClass().getName());
			}
			throw new ShutdownException(
					"Invalid Method Invocation - Invocation Target - on "
							+ aggMethod.getName() + "  " + e2.getMessage());
		}
	}
