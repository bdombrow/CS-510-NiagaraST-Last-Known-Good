/**********************************************************************
  $Id: PhysicalPresent.java,v 1.1 2007/04/28 21:47:13 jinli Exp $
...
public class PhysicalPresent extends PhysicalOperator {
    ...
    protected void processPunctuation(Punctuation tuple, int streamId)
		...
		/*
		 * for each side, we assume 1) its data granularity and its punctuation granularity are the same;
		 * 							2) one's granularity divides the other's
		 */   
		if (granularity[otherStreamId] >= granularity[streamId]) {
			...
			String lastPunctVal = ts[streamId].getAtomicValue(left, right).trim();
			if (lastPunctVal.startsWith("(")) 
			   lastPunctVal = lastPunctVal.substring(2, lastPunctVal.length() - 1);  
			
			long lastPunctTS = Long.valueOf(lastPunctVal);
			
			for (long i= lastPunctTS+granularity[0]; i <= punctTS; i=i+granularity[0]){
