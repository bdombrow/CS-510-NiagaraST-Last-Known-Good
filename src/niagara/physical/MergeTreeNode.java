...
class MergeTreeNode {
	 ...
	 * 
	 * @param eMElt
	 *            ElementMerge element which contains the merge specification to
	 *            be translated to this MergeTreeNode
	 * @param treeMergeType
	 *            The MergeType (inner, outer, etc) specified at the tree level
	 *            (default for the whole tree)
	 * @param treeDomSide
	 *            The Dominant Side (left/right) specified at the tree level
	 *            (default for the whole tree)
	 * @param checkAccumConstraints
	 *            true/false depending on whether we should verify that this
	 *            MergeTemplate is valid for use in accumulator. Accumulate has
	 *            certain restrictions beyond those for a general merge
	 *            template, if checkAccumConstraints is true and these
	...
	MergeTreeNode(Element eMElt, int treeMergeType, int treeDomSide,
		...
		/*
		 * First get the tag names: ResultName, LInputName, RInputName These are
		 * specified as attributes
		 */
		setTagNames(eMElt, checkAccumConstraints);

		/*
		 * The first element of an ElementMerge will be either a Match Template
		 * (optional) or an element describing the merge itself (ShallowContent,
		 * DeepReplace, etc.)
		 */
		Element contentMerge = null;
		localKey = new LocalKey();
		if (DOMHelper.getFirstChildElement(eMElt).getNodeName().equals(
				"MatchTemplate")) {
		...
		/*
		 * the ElementMerge element always contains an element to describe how
		 * the merge should be done
		 */
		...
		/*
		 * The rest of the elements are MergeNodes for the children - create the
		 * children by recursively calling the constructor on appropriate child
		 * elements of ElementMerge element
		 */

		/* create the children!! */
		Element childEMElt = DOMHelper.getNextSiblingElement(contentMerge);
		while (childEMElt != null) {
			/* put a new MergeTreeNode in the children list */
			MergeTreeNode temp = new MergeTreeNode(childEMElt, treeMergeType,
					treeDomSide, checkAccumConstraints, mergeTree);
			children.add(temp);
	...
	/**
	 * Internal function to set the result, lInput and rInput tag names. Does so
	 * on the basis of attributes from the ElementMerge element
	 * 
	 * @param eMElt
	 *            The DOM element representing the specification of the merge
	 *            for this element
	 */
	private void setTagNames(Element eMElt, boolean checkAccumConstraints)
			throws MTException {
		/*
		 * ResultName is required - if it doesn't exist is an error This is
		 * specified in the DTD and so we shouldn't get null ResultName
		 * attribute here
		 */
		...
		if (checkAccumConstraints && !(lInputTagName.equals(resultTagName))) {
			throw new MTException(
					"lInputName doesn't match Name - not valid for accumulate");
		}
		...
		/*
		 * if this is for an accumulate operator, set up the accumTag and
		 * fragTag names
		 */
	...
	/**
	 * Returns the tag name of the accumulator element in the merge represented
	 * by this MergeTreeNode.
	 * 
	 */
	...
	/**
	 * Returns the tag name of the fragment element in the merge represented by
	 * this MergeTreeNode.
	 */
	 ...
	 * 
	 * @param fragName
	 *            String to be matched to the child fragment tag name
	 * 
	...
	/**
	 * Indicates if the merge is deep or not and implies whether the recursion
	 * should continue after this merge has been completed
	 * 
	 ...
	 * @param doc
	 *            Document with which result is to be associated
	 * 
	...
	/**
	 * Function to accumulate one Element into another. Will use the merger
	 * member object to do the accumulate.
	 * 
	 * @param accumElt
	 *            Element to be accumulated into (accumulator)
	 * @param fragElt
	 *            Element to be accumulated into accumElt (accumulatee)
	 * 
	 * @return Returns true or false to indicate whether recursion should
	 *         continue or not
	 */
	 ...
	 * create a local key value string
	 * 
	 * @param elt
	...
	public void dump(PrintStream os) {
		os.println("Merge Tree Node: Tags: " + "R:" + resultTagName + ", l:"
				+ lInputTagName + ", r:" + rInputTagName);
		merger.dump(os);
		localKey.dump(os);
		/*
		 * MergeTreeNode child = children.getHead(); while(child != null) {
		 * child.dump(os); child = child.next; }
		 */
	}
