...
public class PhysicalPunctuate extends PhysicalOperator {
	...
	public PhysicalPunctuate(int iDI, Attribute aTimer, Attribute attrData) {
		setBlockingSourceStreams(new boolean[cInput]);
	...
	public void opInitialize() {
		...
		aeTimer.resolveVariables(inputTupleSchemas[1 - iDataInput],
				1 - iDataInput);
		...
		if (attrDataTimer != null) {
			...
			aeData.resolveVariables(inputTupleSchemas[iDataInput], iDataInput);
			vData = new ArrayList();
		}
	}
	...
	throws ShutdownException, InterruptedException, OperatorDoneException {
		...
		if (streamId == iDataInput) {
			...
			if (fOutput) {
				...
				// Buffer the data tuples for a while before the dbLastTimer is
				// set
				if (dblLastTimer == -1)
					;// Jenny tupleBuffer.add(inputTuple); Jenny: currently skip
				// all the tuples before we get the first timestamp
				else {
					/*
					 * if (fLastTimerFirstSet) { for (int i = 0; i <
					 * tupleBuffer.size(); i++) {
					 * appendTimestamp((StreamTupleElement)tupleBuffer.get(i));
					 * putTuple((StreamTupleElement)tupleBuffer.get(i), 0); }
					 * fLastTimerFirstSet = false; } else {
					 * appendTimestamp(inputTuple); putTuple(inputTuple, 0); }
					 */
			...
			if (vTimer.size() != 1)
				throw new PEException(
						"Punctuate requires exactly one timer value");

			/*
			 * if (firstTimestamp) { dblLastTimer = Long.parseLong((String)
			 * vTimer.get(0), 10); dblLastTimer = dblLastTimer * WARP;
			 * firstTimestamp = false; return; }
			 */
			...
			/*
			 * if (dblLastTimer == -1) dblLastTimer = Long.parseLong((String)
			 * vTimer.get(0), 10); else if (tupleDataSample != null){
			 * dblLastTimer = Long.parseLong((String) vTimer.get(0), 10);
	...
	private void appendTimestamp(Punctuation spe, boolean fTSWildcard) {
		Element eleTS = doc.createElement(Punctuate.STTIMESTAMPATTR);
		String stTS;
	...
	private boolean enforcePunctuation(Tuple tuple) {
		...
		if (attrDataTimer != null) {
			...
			// fOutput = (dbl >= dblLastTimer); Jenny: think "==" should not be
			// allowed;
			fOutput = (dbl > dblLastTimer);
			if (fOutput == false)
				System.out
						.println("PT - Tuple matches punctuation that has been output -- dropping it");
		}
	 ...
	 */
	private Punctuation createPunctuation(Tuple inputTuple, ArrayList values) {
		...
		if (attrDataTimer != null)
			iAttrTimer = inputTupleSchemas[iDataInput]
					.getPosition(attrDataTimer.getName());
		Punctuation spe = new Punctuation(false);
		for (int iAttr = 0; iAttr < tupleDataSample.size() - 1; iAttr++) { // "-1"
			// is
			// used
			// to
			// exclude
		...
		/*
		 * for (int iAttr=0; iAttr<tupleDataSample.size() - 1; iAttr++) { //"-1"
		 * is used to exclude the "TIMESTAMP" field Node ndSample =
		 * tupleDataSample.getAttribute(iAttr); nodeType =
		 * ndSample.getNodeType(); if (nodeType == Node.ATTRIBUTE_NODE) { Attr
		 * attr = doc.createAttribute(ndSample.getNodeName());
		 * attr.appendChild(doc.createTextNode("*")); spe.appendAttribute(attr);
		 * } else { String stName = ndSample.getNodeName(); if
		 * (stName.compareTo("#document") == 0) stName = new String("document");
		 * Element ePunct = doc.createElement(stName); if (iAttr != iAttrTimer)
		 * ePunct.appendChild(doc.createTextNode("*")); else
	 ...
	 */
	public Op opCopy() {
		return new PhysicalPunctuate(iDataInput, attrTimer, attrDataTimer);
	}
