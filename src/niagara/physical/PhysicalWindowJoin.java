...
import niagara.logical.WindowJoin;
...
/**
 * This is the <code>PhysicalWindowJoinOperator</code> that extends the basic
...
//	private String[] rgstTValues;
	...
	/**
	 * As we assume linear punctuation on timestamp attribute, we decide whether
	 * a input tuple should be stored by the timestamp value of the newest
	 * punctuation. In the previous implementation, punctuations are assumed to
	 * be on joining attributes.
	 */
	protected void blockingProcessTuple(Tuple tupleElement, int streamId)
		...
		if (!extensionJoin[streamId] || !producedResult) {
			...
			if (size == 0)
				...
				Punctuation punct = (Punctuation) rgPunct[otherStreamId]
						.get(size - 1);
				pos = inputTupleSchemas[otherStreamId].getPosition("timestamp"
						+ (otherStreamId + 1));
				// tsNode = punct.getAttribute(pos);
				tsNode = (BaseAttr) punct.getAttribute(pos);

				// The threshold ts value that tuples with smaller ts on the
				// other input can be purged.
				// "+1" is a hack, to fix the punctuation from data source.
				// ts = Integer.parseInt(tsNode.getFirstChild().getNodeValue())
				// +1 - INTERVAL;
				ts = Integer.parseInt(tsNode.toASCII()) + 1 - INTERVAL;
			}

			pos = inputTupleSchemas[streamId].getPosition("timestamp"
					+ (streamId + 1));
			// tsNode = tupleElement.getAttribute(pos);
			tsNode = (BaseAttr) tupleElement.getAttribute(pos);
			// double tupleTs =
			// Double.parseDouble(tsNode.getFirstChild().getNodeValue());
			double tupleTs = Double.parseDouble(tsNode.toASCII());

			// if the tuples' ts value is smaller than the threshold ts, it
			...
			// System.out.println("KT: not inserting in hash table " +
			// streamId);
		}
	}
		...
		try {
			...
			for (int tup = 0; tup < otherSourceTuples.size();) {
				...
				if (pred.evaluate(leftTuple, rightTuple)) {
					/*
					 * int leftpos =
					 * inputTupleSchemas[0].getPosition("timestamp1"); int
					 * rightpos =
					 * inputTupleSchemas[1].getPosition("timestamp2"); double
					 * leftVal =
					 * Double.parseDouble(leftTuple.getAttribute(leftpos
					 * ).getFirstChild().getNodeValue()); double rightVal =
					 * Double
					 * .parseDouble(rightTuple.getAttribute(rightpos).getFirstChild
					 * ().getNodeValue()); if (Math.abs(leftVal - rightVal) >
					 * INTERVAL) { tup++; continue; }
					 */
	 ...
	 * because it assumes punctuattion on the hashing attributes. Here, we
	 * assume linear punctuation on time. But eventually we should handle linear
	 * punctuation per group. - Jenny
	 */
	protected void processPunctuation(Punctuation tuple, int streamId)
		...
		try {
			...
			int pos = inputTupleSchemas[streamId].getPosition("timestamp"
					+ (streamId + 1));
			// Node tsNode = tuple.getAttribute(pos);
			BaseAttr tsNode = (BaseAttr) tuple.getAttribute(pos);
			// int ts = Integer.valueOf(tsNode.getFirstChild().getNodeValue()) +
			// 1 - INTERVAL;
			// int ts = Integer.valueOf(tsNode.getFirstChild().getNodeValue()) +
			// 1;
			...
			while (keys.hasNext()) {
				...
				while (list.hasNext()) {
					// if ((ts - INTERVAL) >
					// Integer.valueOf(((Tuple)list.next()).getAttribute(pos).getFirstChild().getNodeValue()))
					if ((ts - INTERVAL) > Integer
							.valueOf(((BaseAttr) ((Tuple) list.next())
									.getAttribute(pos)).toASCII()))
						list.remove();
				}
				if (hashEntry.size() == 0)
					keys.remove();
			...
			while (it.hasNext() && !fMatch) {
				...
				tsNode = (BaseAttr) punct.getAttribute(pos);
				// otherTs =
				// Integer.valueOf(tsNode.getFirstChild().getNodeValue()) + 1;
	...
	/**
	 * @param: punctSide indicates the attribute on which the produced
	 *         punctuation will punctuate. For example, when a left-side
	 *         punctuation arrives, it is possilbe to produce a punctuation on
	 *         the ordering attribute of the right side.
	 */
	protected void producePunctuation(Punctuation left, Punctuation right,
			int punctSide) throws ShutdownException, InterruptedException {
		Punctuation result;
