...
public class Hasher {
	...
	/**
	 * This function generates a hash key for a tuple given the list of relevant
	 * attributes. This is NOT a hash code - it is a key to be input into a hash
	 * table. equality of keys => equality of elements this is not true for hash
	 * codes. KT - this hashing stuff is hard to understand. This key created
	 * isn't really a key, but I don't think that matters because of the way the
	 * hash tables are structured - java's native hash table isn't used
	 * directly.
	 * 
	 * @param tupleElement
	 *            The tuple to be hashed on
	 * @param attributeList
	 *            The list of attributes in the tuple to hash on
	 * 
	 * @return The hash key. If any of the attributes in the attribute list do
	 *         not have a hashable value, then returns null.
	 */
	public String hashKey(Tuple tupleElement) {
		...
		for (int att = 0; att < numAttributes; ++att) {
			...
			if (values[att].size() > 1) {
				throw new PEException(
						"More than one atomic value in hashCode eval "
								+ values[att].size());
			} else if (values[att].size() == 0) {
				return null;
			}
