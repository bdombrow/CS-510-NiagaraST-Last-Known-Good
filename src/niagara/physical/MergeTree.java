...
/**
 * Implementation of a <code> MergeTree </code> class which merges two XML
 * documents together. The merge tree is constructed of MergeTreeNodes.
 * MergeTree is a wrapper for the tree constructed of MergeTreeNodes and also
 * handles the treewalking. MergeTreeNodes make up the tree structure and the
 * Merge and Match objects which are members of each MergeTreeNode, do the real
 * merging and matching work.
 * 
 * @version 1.0
 * 
...
public class MergeTree {
	...
	/**
	 * Creates the MergeTree instance from a string. This function is
	 * responsible for parsing the string appropriately and then calling the
	 * create function which takds a DOM document as an argument.
	 * 
	 * @param mergeTemplate
	 *            String representing the XML merge template. Can be a string
	 *            containing the merge template, a file name, or a URI
	 * @param checkAccumConstraints
	 *            true/false depending on whether you want create to verify that
	 *            this tree can be used
	 * 
	...
	public void create(String mergeTemplateStr, boolean checkAccumConstraints)
		...
		try {
			...
			if (mergeTemplateStr.startsWith("<?xml")) {
				p.parse(new InputSource(new ByteArrayInputStream(
						mergeTemplateStr.getBytes())));
			} else {
			...
			throw new MTException("Error parsing merge template: "
					+ e.getMessage() + " err strs " + p.getErrorStrings());
		}
		return;
	}

	/**
	 * Creates the MergeTree instance from a DOM document consisting of the
	 * MergeTemplate. Traverses the merge template and creates the appropriate
	 * MergeTreeNodes.
	 * 
	 * @param mergeTemplate
	 *            DOM document containing the XML merge template
	 * @param checkAccumConstraints
	 *            true/false depending on whether you want create to verify that
	 *            this tree can be used for accumulate - some configurations
	 *            aren't allowed by accumulate and if accumConstraints is true,
	 *            the existence of these conditions will cause an error to be
	 *            thrown
	 * 
	 * @return Returns nothing.
	 */
	private void create(Document mergeTemplate, boolean checkAccumConstraints)
			throws MTException {
		/*
		 * creates root MergeTreeNode and recursively creates MergeTreeNodes
		 * (one for each element in the merge template) to creates the whole
		 * merge tree.
		 */
		...
		mergeTreeRoot = new MergeTreeNode(mergeElt, treeMergeType, treeDomSide,
				checkAccumConstraints, this);
		...
		/*
		 * now that creation succeeded, indicate if this tree is valid to be
		 * used as an accumulate tree or not
		 */
		validForAccumulate = checkAccumConstraints;
		return;
	}

	private void setMergeType(Element docElement) throws MTException {
		setDominantSide(docElement);

		/*
		 * know we won't get empty strings here because these attrs have default
		 * values
		 */
	...
	private void setDominantSide(Element docElement) throws MTException {
	...
	/**
	 * Merges the an XML Element into an XML Document as specified by the merge
	 * tree associated with the class. For now, this is not general enough to
	 * handle the join (merge two fragments and create a new result) - maybe it
	 * will be some day
	 * 
	 * @param accumDoc
	 *            The document into which the fragment will be merged
	 * @param fragment
	 *            The fragment to be merged into the document later this may be
	 *            of type XMLFragment or something - for now it is just a normal
	 *            tree
	 */

	void accumulate(Element fragment) throws ShutdownException {

		/*
		 * System.out.println("--------------------------------------------------"
		 * ); System.out.println("MergeTree.accumulate. Input element ");
		 * DOMHelper.printElt(fragment); System.out.println();
		 * System.out.println();
		 */

		/*
		 * make it left and right frag and make treewalking code general enough
		 * to handle the join code?
		 */
		Element accumFragRoot = accumDoc.getDocumentElement();
		Element mergeFragRoot = fragment;

		/*
		 * have to handle the case when we start with an empty accumulator - in
		 * this case accumFragRoot is null, so we make an empty document element
		 * for the accumulator // and root add it to the tree
		 */
		boolean newRoot = false;
		if (accumFragRoot == null) {
			if (TRACE) {
				System.out
						.println("KT: Empty Accumulator - creating accum root");
			}
			newRoot = true;
		} else {
			if (TRACE) {
		...
		/*
		 * call accumulate. for 2nd argument pass in the key value for the
		 * accumRoot - we pass in an empty RootedKeyVal to createRootedKeyVal
		 * for the key of parent of accumRoot
		 */
		MyStringBuffer accumFragRootKeyVal = getTempStringBuffer();

		mergeTreeRoot.createLocalKeyValue(mergeFragRoot, accumFragRootKeyVal);

		accumFragRoot = do_accumulate(accumFragRoot, accumFragRootKeyVal,
	...
	/**
	 * Recursive function to walk the merge tree, accumulator (result), and the
	 * fragment and do the merge. The actual merging of elements and matching is
	 * done by merge and match objects. The objective of this function is to
	 * handle the tree walking and locating which elements to call merge and
	 * match on.
	 * 
	 * @param accumElt
	 *            The current element of the accumulate document (result)
	 * @param fragElt
	 *            The current element of the fragment
	 * @param mergeTreeNode
	 *            The current node of the merge tree
	 * 
	 */

	private Element do_accumulate(Element accumEltArg,
			MyStringBuffer accumEltKeyVal, Element fragElt,
			MergeTreeNode mergeTreeNode) throws ShutdownException {
		...
		/*
		 * in all cases, I get in two elements that should immediately be
		 * merged, we know that accumElt and fragElt "match" if applicable
		 */

		/*
		 * calls the merger (MergeObject) in the MergeTreeNode to do the merge,
		 * just does locally what is needed to merge these two objects -
		 * recursion is done below
		 */
		...
		if (accumElt == null) {
			accumEmptyCnt++;
			// newAccumElt = true;
			accumElt = mergeTreeNode.accumulateEmpty(fragElt, mergeTreeNode
					.getAccumTagName());
		} else {
		...
		if (!mergeTreeNode.isDeepMerge()) {
			/*
			 * now call recursively - we scan the subelements of fragElt and
			 * look for subelements of the mergeElt with the same tag name - if
			 * found, then get the subelement from accumElt with matching tag,
			 * if available, and make the recursive call.
			 * 
			 * If no existing subelement in accumElt, then do what is
			 * appropriate - insert or ignore based on Merge Type
			 * 
			...
			while (nextFragElt != null) {
				...
				/*
				 * could avoid this wierd lookup by scanning merge tree first -
				 * but that could be inefficient??
				 */
				MergeTreeNode nextMergeTreeNode = mergeTreeNode
						.getChildWithFragTagName(nextFragTagName);

				/* skip the fragment if there is no merge info for it */
				if (nextMergeTreeNode != null) {

					String nextAccumTagName = nextMergeTreeNode
							.getAccumTagName();
					...
					if (nextMergeTreeNode.isNever()) {
						...
						if (!nextMergeTreeNode.isTag()) {
							addChildrenToRootedKeyMap(accumElt, accumEltKeyVal,
									nextAccumTagName, nextMergeTreeNode);
						}
						 ...
						 */
						MyStringBuffer nextAccumEltKeyVal = getTempStringBuffer();
						nextMergeTreeNode.createLocalKeyValue(nextFragElt,
								nextAccumTagName, nextAccumEltKeyVal);
						nextAccumEltKeyVal.append(LocalKey.PARENT_SEPARATOR);
						nextAccumEltKeyVal.append(accumEltKeyVal);

						/*
						 * We assume that only one child of accumFrag will match
						 * the nextFragElt - although there might be multiple
						 * children of accumFrag that have the tag accumName
						 */
						 ...
						 */
						if (!nextMergeTreeNode.isTag()) {
							nextAccumElt = (Element) rootedKeyMap
									.get(nextAccumEltKeyVal);
						} else {
							nextAccumElt = DOMHelper.getFirstChildEltByTagName(
									accumElt, nextAccumTagName);
						}
						...
						/* the recursive call */
						Element newNextAccumElt = do_accumulate(nextAccumElt,
								nextAccumEltKeyVal, nextFragElt,
								nextMergeTreeNode);

						assert (newNextAccumElt != nextAccumElt && expectNew)
								|| (newNextAccumElt == nextAccumElt && !expectNew) : "KT - problem with empty accumulator case";
						 ...
						 * call is completed (have to get latching correct) Not
						 * valid any more... KT
						 */
						/* done with child */
						...
						if (expectNew) {
							...
							addChildToRootedKeyMap(newNextAccumElt,
									accumEltKeyVal, nextMergeTreeNode, false);
						}
					}
				}
	 ...
	 * 
	 * @param parent
	 *            Parent whose children are to be inserted into hash map
	 * @param parentKeyVal
	 *            The rootedKey value of the parent
	 * @param tagName
	 *            Only children with this tag name are to be added to the hash
	 *            map
	 * @param mergeTreeNode
	 *            The mergeTreeNode containing key information for the child
	 */
	private void addChildrenToRootedKeyMap(Element parent,
			MyStringBuffer parentKeyVal, String tagName,
			MergeTreeNode mergeTreeNode) throws ShutdownException {
		...
		ListIterator childrenIter = (DOMHelper.getChildElementsByTagName(
				parent, tagName)).listIterator();

		while (childrenIter.hasNext()) {
			addChildToRootedKeyMap((Element) childrenIter.next(), parentKeyVal,
					mergeTreeNode, true);
		}
	...
	/**
	 * addChildToRootedKeyMap adds a parent's child to the rootedKeyMap
	 * 
	 * @param elt
	 *            The element which is to be inserted into rooted key map
	 * @param parentKeyVal
	 *            The parent's key value
	 * @param mergeTreeNode
	 *            The merge tree node with key information necessary for
	 *            creating the child's rooted key
	 * @param multiKey
	 *            True if this child is allowed to have multiple key - values -
	 *            this allows us to lift the restriction that matches aren't
	 *            allowed on set values
	 */
	private void addChildToRootedKeyMap(Element child,
			MyStringBuffer parentKeyVal, MergeTreeNode mergeTreeNode,
			boolean multiKey) throws ShutdownException {
		...
		if (!mergeTreeNode.isTag()) {
			if (!multiKey) {
				/*
				 * If necessary, create the rooted key value for the child and
				 * put it in the rootedKeyMap
				 */
	...
	/**
	 * Merges two XML Documents together to produce a new result XML Document as
	 * specified by the merge tree associated with this class.
	 * 
	 * @param lDoc
	 *            "left" document to be merged
	 * @param rDoc
	 *            "right" document to be merged
	 * 
