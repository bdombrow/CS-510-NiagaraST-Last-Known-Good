...
public class PhysicalUnnest extends PhysicalOperator implements NodeConsumer {
	...
	protected void processTuple(Tuple inputTuple, int streamId)
		...
		if (inputTuple.getAttribute(scanField) instanceof Node)
			...
			pev.produceMatches(((XMLAttr) inputTuple.getAttribute(scanField))
					.getNodeValue(), this);
		else
		...
		if (!matchFound) {
			if (outer == Unnest.OuterBehavior.OUTER)
				...
				throw new ShutdownException("Input does not contain '"
						+ variable + "'");
		}
	...
	public void consume(Node n) throws InterruptedException, ShutdownException {
	...
	public int hashCode() {
		return path.hashCode() ^ variable.hashCode() ^ root.hashCode()
				^ outer.hashCode();
	}
	 ...
	 *                query shutdown by user or execution error
	 */
	protected void processPunctuation(Punctuation punct, int streamId)
			throws ShutdownException, InterruptedException {
		...
		if (inputPunct.getAttribute(scanField) instanceof Node)
			...
			pev.produceMatches(((XMLAttr) inputPunct.getAttribute(scanField))
					.getNodeValue(), this);
		else
			throw new PEException("JL: We only unnest xml node or xmlattr");
	 ...
	 * throws ShutdownException, InterruptedException { // XXX vpapad: Pete, I
	 * don't know how to modify this // to handle projections - HELP try { //
	 * Get the attribute to scan on Node attribute =
	 * (Node)inputTuple.getAttribute(scanField);
	 * 
	 * // Get the nodes reachable using the path expression scanned
	 * pev.getMatches(attribute, elementList);
	 * 
	 * // Append all the nodes returned to the inputTuple and add these // to
	 * the result int numNodes = elementList.size();
	 * 
	 * if (numNodes != 0) { for (int node = 0; node < numNodes; ++node) { //
	 * Clone the input tuple to create an output tuple // Append a reachable
	 * node to the output tuple // and put the tuple in the output stream
	 ...
	 * putTuple(outputTuple, 0); } } else { //I still want the punctuation to
	 * live on, even if it doesn't // have the element we're scanning for.
	 * putTuple(inputTuple, streamId); } elementList.clear(); } catch
	 * (java.lang.ArrayIndexOutOfBoundsException ex) { //the scan field doesn't
	 * exist for this punctuation. We // still want the tuple to live on.
