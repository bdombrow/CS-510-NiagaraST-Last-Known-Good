...
import niagara.magic.MagicContentAttr;
import niagara.magic.MagicContentElement;
import niagara.magic.MagicNode;
import niagara.magic.MagicShellElement;
...
/**
 * <code>PhysicalMagicConstruct</code> is an implementation of the Construct
 * operator.
 */
public class PhysicalMagicConstruct extends PhysicalOperator {
	...
	private void buildTemplate() throws ShutdownException {
		...
		assert resultList.size() == 1 : "HELP got " + resultList.size()
				+ " results - expected only 1";

		magicResult = resultList.get(0);
	}

	private void buildTempl(NodeVector localResult,
			constructBaseNode constructNode, boolean isRoot)
			throws ShutdownException {
		if (constructNode instanceof constructLeafNode) {
			buildLeafNodeTempl((constructLeafNode) constructNode, localResult,
					isRoot);
	...
	private void buildLeafNodeTempl(constructLeafNode leafNode,
			NodeVector localResult, boolean isRoot) {
		...
		case dataType.STRING:
			...
			localResult.add(doc.createTextNode((String) leafData.getValue()));
			break;
		...
		case dataType.ATTR:
			...
			int attrIdx = ((schemaAttribute) leafData.getValue()).getAttrId();
			...
			case varType.CONTENT_VAR:
				...
				MagicContentElement magicContent = new MagicContentElement(
						attrIdx, magicRoot, null);
	...
	private void buildInternalNodeTempl(constructInternalNode internalNode,
			NodeVector localResult, boolean isRoot) throws ShutdownException {
		...
		MagicBaseNode resultElement = new MagicShellElement(tagName, magicRoot);
		if (isRoot) {
		...
		for (int i = 0; i < numChildren; i++) {
			...
			buildTempl(localResult, (constructBaseNode) children.get(i), false);
	...
	public void addAttributes(constructInternalNode internalNode,
			Element resultElement) throws ShutdownException {
		...
		for (int i = 0; i < attrs.size(); i++) {
			...
			case dataType.ATTR:
				...
				int attrIdx = ((schemaAttribute) attrData.getValue())
						.getAttrId();
				...
				case varType.CONTENT_VAR:
					MagicContentAttr magicContentAttr = new MagicContentAttr(
							attrIdx, magicRoot, attrName);
