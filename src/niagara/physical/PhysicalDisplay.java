...
public class PhysicalDisplay extends PhysicalOperator {
	...
	public boolean equals(Object o) {
		...
		return query_id.equals(other.query_id)
				&& url_location.equals(other.url_location);
	}
	...
	protected void processTuple(Tuple tupleElement, int streamId)
		...
		if (counter == 0) {
			try {
				...
				throw new ShutdownException("Bad url for client "
						+ mue.getMessage());
			} catch (java.io.IOException ioe) {
				throw new ShutdownException(
						"Unable to open connection to client " + url_location
								+ " " + ioe.getMessage());
			}
		}
	...
	protected void cleanUp() {
		try {
			...
			System.err
					.println("Display: error while sending results to client:"
							+ url_location);
			e.printStackTrace();
			System.exit(-1);
		}
	...
	public Cost findLocalCost(ICatalog catalog, LogicalProperty[] InputLogProp) {
		// Setup costs for opening a connection, plus a fixed cost per
		// transmitted tuple
		Cost final_cost = new Cost(catalog.getDouble("open_connection_cost"));
		Cost tuple_transmission = new Cost(catalog
				.getDouble("tuple_transmission_cost"));
		final_cost.add(tuple_transmission.times(InputLogProp[0]
				.getCardinality()));
		return final_cost;
	}
