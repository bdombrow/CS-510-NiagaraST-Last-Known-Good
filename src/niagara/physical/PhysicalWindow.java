...
/**********************************************************************
  $Id: PhysicalWindow.java,v 1.1 2003/12/24 01:49:01 vpapad Exp $
...
public abstract class PhysicalWindow extends PhysicalGroup {
...
/*    public PhysicalWindowOperator(op logicalOperator,
    ...
    protected Tuple createTuple (Node groupedResult,
					    Tuple representativeTuple,
					    boolean partial) {

	Tuple tupleElement =super.createTuple(groupedResult, 
    ...
    protected void updateWindowStatus (Tuple tupleElement, int streamId) {
	...
	if ((everyCount != 0) && (everyCount % every) == 0) {
		...
		for (int i = 0; i < size; i++) 
			super.blockingProcessTuple(((Tuple) window.get(i)), 
