...
class MTNList {
	...
	/*
	 * private void put(String fragName, MergeTreeNode entry) { int hashVal =
	 * hashCode(fragName); assert hashVal < hashTable.length && hashVal >= 0 :
	 * "hashVal " + hashVal + " len " + hashTable.length; entry.next =
	 * hashTable[hashVal]; hashTable[hashVal] = entry; }
	 * 
	 * private MergeTreeNode get(String fragName) { int hashVal =
	 * hashCode(fragName);
	 * 
	 * MergeTreeNode curr = hashTable[hashVal]; while(curr != null) {
	 * if(curr.getFragTagName().equals(fragName)) return curr; curr = curr.next;
	 * } return null; }
	 * 
	 * private int hashCode(String fragName) { int hc = fragName.hashCode() %
	 * TABLE_SIZE; if(hc < 0) return 0-hc; else return hc; }
	 */
}
