...
/**
 * This is the <code>PhysicalGroupOperator</code> that extends the basic
 * PhysicalOperator with the implementation of the group operator.
 * 
 * @version 1.0
 * 
 */
...
public abstract class PhysicalGroup extends PhysicalOperator {
	...
	public final boolean equals(Object o) {
		...
		if (!groupAttributeList.equals(((PhysicalGroup) o).groupAttributeList))
			return false;
	...
	protected void blockingProcessTuple(Tuple tupleElement, int streamId)
		...
		BaseAttr ungroupedResult = this.constructUngroupedResult(tupleElement);
	...
	/**
	 * This function handles punctuations for the given operator. The group-by
	 * operator can unblock itself when a punctuation arrives that matches a
	 * group (or groups). Groups that match can be output, and their state can
	 * be purged.
	 * 
	 * @param tuple
	...
	protected void processPunctuation(Punctuation inputTuple, int streamId)
		...
		while (en.hasMoreElements()) {
			...
			for (int i = 0; i < rgstPValues.length && fMatch; i++) {
				fMatch = Punctuation.matchValue(rgstPValues[i], rgstTValues[i]);
			}
	 ...
	 */
	// HERE
	protected Tuple createTuple(Node groupedResult, Tuple representativeTuple,
			boolean partial) {
	 ...
	 * throws ShutdownException;
	 */
	protected abstract BaseAttr constructUngroupedResult(Tuple tupleElement)
			throws ShutdownException;
	...
	/*
	 * protected abstract Object mergeResults( Object groupedResult, Object
	 * ungroupedResult);
	 */
	...
	/*
	 * protected abstract Node constructResult( Object partialResult, Object
	 * finalResult);
	 */
