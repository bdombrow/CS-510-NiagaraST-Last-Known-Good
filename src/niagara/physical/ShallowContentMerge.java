...
import niagara.utils.type_system.NodeHelpers;
import niagara.utils.type_system.NumberNodeHelper;
...
class ShallowContentMerge extends MergeObject {
	...
	/**
	 * Constructor for ShallowContentMerge - reads in a ShallowContent DOM
	 * element from the XML merge tree
	 */
	ShallowContentMerge(Element shallowContElt, int treeDomSide,
			int treeMergeType, MergeTree mergeTree) throws MTException {
		if (shallowContElt.getNodeName().equals("ExactMatch")) {
			createExactMatchMerge();
		}

		contentMerge = createNodeMerge(shallowContElt, treeDomSide,
				treeMergeType);

		attrMergesMap = new HashMap();

		/*
		 * Go through chidren of shallowContElt and create attribute merging
		 * info
		 */
		String attrName;
		...
		Element attrMergeElt = DOMHelper.getFirstChildElement(shallowContElt);
		while (attrMergeElt != null) {
			attrName = attrMergeElt.getAttribute("Name");
			attrShallowContElt = DOMHelper.getFirstChildElement(attrMergeElt);
			if (attrShallowContElt != null) {
				attrNodeMerge = createNodeMerge(attrShallowContElt,
						treeDomSide, treeMergeType);
				/* just for safety */
				assert DOMHelper.getNextSiblingElement(attrMergeElt) == null : "KT: Unexpected sibling";
			} else {
				/*
			...
			attrMergeElt = DOMHelper.getNextSiblingElement(attrMergeElt);
		}
	...
	private NodeHelper getAppropriateNodeHelper(Element shallowContElt)
		...
		try {
			NodeHelper ret = NodeHelpers.getAppropriateNodeHelper(lValueType,
					rValueType);
			return ret;
		} catch (MTException mte) {
			throw new MTException("Shallow Content Merge ("
					+ shallowContElt.getTagName() + ") " + mte.getMessage());
		}
	}

	private NodeMerge createNodeMerge(Element shallowContElt, int treeDomSide,
			int treeMergeType) throws MTException {
		...
		if (shallowContElt.getTagName().equals("ExactMatch")
			...
			assert comparator instanceof NumberNodeHelper : "Invalid Node Helper - need NumberNodeHelper here";
	...
	private void createExactMatchMerge() {
		...
		NodeMerge attrNodeMerge = new ExactMatchNodeMerge(
				NodeHelpers.STRINGHELPER);
	 ...
	 * 
	 * @param _contentMerge
	 *            The new merge object for the content
	 */
	/*
	 * void addContentMerge(NodeMerge _contentMerge) { contentMerge =
	 * _contentMerge; return; }
	 */
	 ...
	 * 
	 * @param attrName
	 *            The name of the attribute for which NodeMerge is changing
	 * @param attrMerge
	 *            The new NodeMerge object
	 */
	/*
	 * void addAttrMerge(String attrName, NodeMerge attrMerge) { remove any
	 * previous NodeMerge for this attribute * attrMergesMap.remove(attrName);
	 * attrMergesMap.put(attrName, attrMerge); return; }
	 */
	 ...
	 * 
	 * @param accumElt
	 *            The element to be accumulated into (accumulator)
	 * @param fragElt
	 *            The element to accumulate into accumElt (accumulatee)
	 * 
	 * @return Returns nothing.
	 */
	void accumulate(Element accumElt, Element fragElt) throws ShutdownException {
		/*
		 * traverse through all attributes and content - if an attribute value
		 * (or node content) matches, no action. If a new attribute/content is
		 * found in fragElt, it is inserted into accumElt. If content/attribute
		 * value doesn't match, the value from the "dominant" side is used.
		 */
	...
	Element accumulateEmpty(Element fragElt, String accumTagName)
		...
		/*
		 * Handle the case with an empty accumElt here. This may occur when we
	 ...
	 * 
	 * @param lElt
	 *            "left" element to be merged
	 * @param rElt
	 *            "right" element to be merged
	 * @param resDoc
	 *            The document with which the result element is to be associated
	 * @param tagName
	 *            The tag name for the new result element
	 * 
	 * @return Returns new result element
	 */
	Element merge(Element lElt, Element rElt, Document resDoc, String tagName)
			throws ShutdownException {
	 ...
	 * result element. This is used internally and captures both the accumulate
	 ...
	 *            "right" element to be merged
	 * @param resElt
	 *            Result element - this elt will be modified and upon return
	 *            will contain the result of the merge Note - attrs are added to
	 *            resElt and any existing attrs will be preserved (needed for
	 *            accumulate)
	 * 
	 * @return True to indicate that recursion should continue
	 */
	private void internal_merge(Element lElt, Element rElt, Element resElt)
			throws ShutdownException {
		...
		/*
		 * for efficiency - just do this test once - we may have one of these
		 * cases when we are merging
		 */
	...
	/**
	 * Function to merge the attributes of two elements. One element is always
	 * dominant - that element's content is put in result and if the two
	 * elements have any of the same attributes - the attribute value from the
	 * dominant element is taken. As for attributes that appear in one element,
	 * but not in the other, several options are supported - left outer "join",
	 * right outer "join", inner "join", and outer "join"
	 * 
	 * The result element is modified appropriately.
	 * 
	 * Maybe should turn this into a HashJoin class...
	 */
	private void mergeAttributes() throws ShutdownException {
		/*
		 * create hash maps from the attributes - I don't use NamedNodeMap
		 * because I don't want attributes with default values to be replaced,
		 * and because I don't want to change the state of leftElt and rightElt
		 * Questionable - would like to leave accumAttrsMap around in accum
		 * case...
		 */
		...
		/*
		 * array of attributes to be added to, changed in and delete from the
		 * result element
		 */
		addAttrs.clear();
		changeAttrs.clear();
		...
		if (numLAttrs == 0 && numRAttrs == 0) {
			...
			processAttrsAsOuter(rAttrsMap, false, rightIsResult, addAttrs,
					delAttrs);
		} else if (numRAttrs == 0) {
			processAttrsAsOuter(lAttrsMap, true, leftIsResult, addAttrs,
					delAttrs);
		} else {
			/*
			 * join the attrMaps - join condition is equality of attribute names
			 * look for matches in the maps - if a match found - then proceed to
			 * merge the matching attributes - if no match found, process the
			 * "outer" attribute appropriately
			 */
			...
			for (int i = 0; i < numRAttrs; i++) {
				...
				/*
				 * Ignore all attributes without any merge information - by
				 * removing them from the appropriate attrsMap
				 */
				NodeMerge nodeMerge = (NodeMerge) (attrMergesMap.get(rAttr
						.getName()));

				/*
				 * handle the default (ATTR_DEFAULT). If a default is provided,
				 * it applies to all attributes that haven't had AttributeMerge
				 * elements explicitly specified.
				 */
				if (nodeMerge == null) {
				...
				if (nodeMerge == null) {
					...
					Attr lAttr = null;
					lAttr = (Attr) (lAttrsMap.getNamedItem(rAttr.getName()));
					...
					if (lAttr != null) {
						 ...
						 */
						Attr tempResultAttr = resultElt.getOwnerDocument()
								.createAttribute(lAttr.getName());
						boolean new_result;
						...
						/*
						 * lAttr is null-means rAttr is a "Right Outer"
						 * attribute
						 */
			...
			/*
			 * Process remaining attributes from the left side an attribute is
			 * "remaining" if it hasn't already been processed and put in the
			 * procLeftAttrs list
			 */
			...
			for (int i = 0; i < len; i++) {
				...
				/*
				 * Ignore all attributes without any merge information - by not
				 * doing anything
				 */
				NodeMerge nodeMerge = (NodeMerge) (attrMergesMap.get(attr
						.getName()));
				if (nodeMerge != null) {
		...
		for (int i = 0; i < len; i++) {
			...
			Attr toAddAttr = (Attr) (mergeTree.accumDoc.importNode(addedAttr,
					false));
		...
		for (int i = 0; i < len; i++) {
			...
			/*
			 * a wont be null, since to change, we know we had attr values from
			 * both sides
			 */
			if (a == null)
	...
	/**
	 * Process all attributes in a map from a side. For some reason we know that
	 * these attrs do not have a match on the other side - this can be because
	 * the other attrsMap is null or because all matching attrs have already
	 * been processed
	 * 
	 * @param attrsMap
	 *            The hash map of the attributes to be processed
	 */
	private void processAttrsAsOuter(NamedNodeMap attrsMap, boolean isLeft,
			boolean isResult, ArrayList resultAttrs, ArrayList deletedAttrs) {
		...
		for (int i = 0; i < numAttrs; i++) {
			 ...
			 * doing anything
			 */
			NodeMerge nodeMerge = (NodeMerge) (attrMergesMap
					.get(attr.getName()));
			if (nodeMerge != null) {
				...
				/*
				 * lAttr is null-means attr is a "1Outer" attribute
				 */
				processOuterAttr(attr, includeOuter, isResult, resultAttrs,
						deletedAttrs);
			}
		}
	}

	private void mergeContent() throws ShutdownException {
		contentMerge.merge(leftElt, rightElt, resultElt);
		return;
	}

	/**
	 * Create a hash map from an array of attributes - the key is the attribute
	 * name, the value is the attribute itself
	 * 
	 * @param attributeArray
	 *            The array of attributes to put into the hash map
	 * 
...
//	}
	...
	/**
	 * Function to process an attribute which doesn't have a match in the other
	 * element. Call this an "outer" attribute as in outer join terminology -
	 * which is basically what is going on here
	 * 
	 * @param attr
	 *            The attribute to be processed
	 * @param includeOuter
	 *            Indicates if this attr should be included in result even if
	 *            there is no matching attr on other side
	 * @param isResult
	 *            Indicates if elt is the actual result elt or not
	 * @param resultAttrs
	 *            The place to put any attrs that should be added to the result
	 *            (use a list to avoid haloween problem-like effects
	 * @param elt
	 *            The element associated with this attr
	 * 
	 */
	private void processOuterAttr(Attr attr, boolean includeOuter,
		 ...
		 * isResult && !inclOuter - remove attr from result !isResult &&
		 * inclOuter - add attr to result !isResut && !inclOuter - nothing to do
		 * - attr isn't in result and shouldn't be
		 */
