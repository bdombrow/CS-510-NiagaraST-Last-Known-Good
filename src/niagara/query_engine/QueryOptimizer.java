...
import niagara.data_manager.DTDInfo;
...
public class QueryOptimizer {
	 ...
	 * 
	 * @param dataManager
	 *            The data manager that the query optimizer can contact if
	 *            necessary
	 */
	 ...
	 * logical plan
	 * 
	 * @param unoptimizedPlan
	 *            The unoptimized logical plan that is to be optimized
	 * 
	 * @return The optimized logical plan. If there is an error during
	 *         optimization return null.
	 */
	 ...
	 * 
	 * @param logicalPlan
	 *            The logical plan describing the query
	 * 
	...
	private boolean getStatisticsAndUrls(LogNode logicalPlan)
		...
		// them and dtdScanOp nodes so that we can find urls for them
			...
			boolean proceed = collectDTDsAndSelectNodes(logicalPlan, dtdVector,
					selectNodes);
	...
	/**
	 * This function returns the list of DTDs reachable from the given node of
	 * the logical plan that do not have specific URLs to query over.
	 * 
	 * @param logPlanRoot
	 *            The root of the logical plan of interest
	 * @param dtdVector
	 *            The vector to add DTD that do not have specific URLs to query
	 *            over
	 * @param dtdPredicateVector
	 *            search engine predicates
	 * 
	...
	private boolean collectDTDsAndSelectNodes(LogNode logPlanRoot,
		...
		if (operator instanceof DTDScan) {
			...
			// recurse
			for (int i = 0; i < numInputs; i++) {
				proceed = collectDTDsAndSelectNodes(logPlanRoot.input(i),
						dtdVector, selectNodes);
			}
	...
	/**
	 * This function adds the dtd scan operator to the list if it has URLs that
	 * are not listed explicitly
	 * 
	 * @param dtdScanOperator
	 *            The dtd scan operator to be checked
	 * @param dtdVector
	 *            The vector to which the dtd scan operator is to be added if it
	 *            has inexplictly specified URLs
	 * 
...
//		int numRep = urlRep.size();
	 ...
	 * @param logPlanRoot
	 *            The root of the logical plan of interest
	 * @param dtdInfoHashTable
	 *            Hashtable containing information about each DTD, with the DTD
	 *            id as the key
	 * 
	 * @return True if no error occurred, false otherwise
	 */

	private boolean updateDTDInfoWithURLs(LogNode logPlanRoot, DTDInfo dtdInfo)
			throws PushSEQueryException, NoDataSourceException {
	 ...
	 * 
	 * @param dtdScanOperator
	 *            The dtd scan operator to be updated
	 * @param dtdInfoHashTable
	 *            Hashtable containing information about each DTD, with the DTD
	 *            id as the key
	 * 
	...
	private boolean updateInfoWithURLs(DTDScan dtdScanOperator, DTDInfo dtdInfo)
			throws NoDataSourceException {
	...
	/**
	 * This function selects algorithms for various operators. This represents
	 * primitive optimization
	 * 
	 * @param logicalPlan
	 *            The logical plan for which algorithms have to be selected for
	 *            each operator
	 * 
	 ...
	 * 
	 * @param joinOperator
	 *            The join operator for which an algorithm is to be selected
	 */

	private void selectJoinAlgorithm(Join joinOperator) {

		// If there is an equi-join component, then use hash join (join
		// algorithm
		// index of 1), else use nested loop join (join algorithm index of 0).
		// Note
		// that in the future, one will not make explicit references to
		// index-algo
		// associations. Each join algorithm will have a cost and that will be
		// used
		// to select the appropriate join. Thus, in the case of non-equi join,
		// the
		// hash join operator will have infinite cost and will not be selected.
		// No
		// reference should be made to the ordering of join algorithms as is
		// done
		// now.
		//
