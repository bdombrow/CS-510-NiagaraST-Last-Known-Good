...
/**
 * This class handles responses from the server and demultiplexes the resutls
 * for the various queries that will be run on the client and sent to the
 * server.
 * 
 */
...
class ResponseHandler extends HandlerBase {
	...
	/**
	 * Children list of Ell's to construct the result tree for display
	 */
	private List childrenList = new LinkedList();
	/**
	 * This is the list of tree nodes that will end up in the registry
	 */
	 ...
	 * @param obj
	 *            the object to notify for changes
	 */
	 ...
	 *            the attribute value
	 * @param isSpecified
	 *            false if the DTD default is used
	 */
	...
	public void endElement(String elname) throws Exception {
		...
		if (ps == INI) {
			...
			int serverId = Integer
					.parseInt((String) (ell.attrs.get(SERVER_ID)));
			...
			if (resultLevel != 0) {
				...
				DefaultMutableTreeNode elRoot = new DefaultMutableTreeNode(
						elname, true);
				...
				if (ell.attrs != null) {
					...
					while (attrValueSetIt.hasNext()) {
						...
						DefaultMutableTreeNode attChild = new DefaultMutableTreeNode(
								treeValue);
						elRoot.add(attChild);
					}
				}
				...
				while (listIt.hasNext()) {
					...
					if (e.name == null) {
						// This is a text node
						DefaultMutableTreeNode cdata = new DefaultMutableTreeNode(
								e.charData);
						elRoot.add(cdata);
						listIt.remove();
					} else {
				...
				parserState = MSG;
				// Create a list of mutable tree nodes the children list to add
				// to the registry
				...
				while (ci.hasNext()) {
					...
					if (e.data != null) {
						...
						// Create a tree node
						nodeList.add(new DefaultMutableTreeNode(e.charData));
					}
				}
				childrenList.clear();
			}
	...
	public void error(String message, String systemId, int line, int column)
		...
		System.err.println("  at " + systemId + ": line " + line + " column "
				+ column);
	 ...
	 *            the result type
	 * @param nList
	 *            the list of result nodes
	 */
	public void updateRegistry(int lid, int sid, String rtype, List nList) {
		...
		} else // General Error
		{
			...
			DefaultMutableTreeNode n = new DefaultMutableTreeNode("Error");
