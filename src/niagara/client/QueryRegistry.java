...
/**
 * This class holds the result trees for all the active queries the active
 * queries in the client.
 */
@SuppressWarnings("unchecked")
class QueryRegistry {
	// private variables
	/**
	 * The map: LocalId's -> Result Trees and Control information
	 */
	...
	public class Entry {
		...
		public Query query;
		/**
		 * This is the id on the server side and is used on the server side to
		 * route control msgs. It is -1 initially. A value <0 denotes a dont
		 * care id
		 */
		...
		/**
		 * @return a valid server id for this query, wait()ing for the server
		 *         response if necessary
		 */
		// KT-VPAPAD - debug statements for client deadlock on error
		public int getServerId() throws ClientException {
			// System.out.println("DLDEBUG: Thread " +
			// Thread.currentThread().getName() +
			// " requesting mutex on QueryRegistry");
			synchronized (this) {
				// System.out.println("DLDEBUG: Thread " +
				// Thread.currentThread().getName() +
				// " acquired mutex on QueryRegistry");
				try {
					// KT-VPAPAD
					// System.out.println("DLDEBUG: Thread " +
					// Thread.currentThread().getName() +
					// "waiting on QueryRegistry - releasing mutex");
					if (serverId == -1)
						wait();
				...
				if (queryHadError || serverId == -1)
					throw new ClientException(
							"Query Error - possibly in parsing query");
				return serverId;
			}
		}
	 ...
	 * 
	 * @param id
	 *            the id of the query which will be used for the map
	 * @param qString
	 *            query string
	 ...
	 * 
	 * @param id
	 *            the id of the query to which to add results
	 * @param sid
	 *            the server id as sent by the server
	 * @param results
	 *            an Enumeration of DefaultMutableTreeNodes
	 * @param isFinal
	 *            indicate whether this is the final result
	 */
	public synchronized void addResults(int id, int sid, List results) {
		...
		if (e.serverId < 0) {
			...
			System.err.println("ID mismatch: Server sent " + sid
					+ " Registry has " + e.serverId);
		}
	 ...
	 *            the query id.
	 * @param node
	 *            a DefaultMutableTreeNode object that gets the result tree
	 * @param qString
	...
	/**
	 * Check to see if the query was killed so that you don't notify the client
	 * needlessly
	 * 
	 * @param id
	 *            the query id
	...
	/**
	 * This method removes a query from the query list. The removal happens on
	 * request by the gui
	 * 
	 * @param id
	 *            the query id
