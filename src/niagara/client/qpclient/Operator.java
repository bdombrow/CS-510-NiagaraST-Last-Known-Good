...
public class Operator extends QPNode {
    ...
    public void parse(Element e, IDREFResolver idr, 
		      GraphModel gm) {
    ...
    /**
     * <code>handleAttributes</code> can be overriden
     * to specify the way attributes are handled
     * the default implementation registers all such 
     * attributes as properties.
     *
     * @param e a DOM <code>Element</code> 
     * @param idr an <code>IDREFResolver</code> 
     */
	...
	lines = new LineDescription[] {new LineDescription(getType(), CENTER, null)};
    }
    ...
    public Rectangle2D manageSize(LineDescription lines[], QPFigure qpf, Graphics2D g) {
	...
	for (int i = 0; i < lines.length; i++) {
	    ...
	    Rectangle2D.Float b = (Rectangle2D.Float) 
		g.getFontMetrics().getStringBounds(lines[i].line, g);
	...
	for (int i=0; i < lines.length; i++) {
	    ...
	    case CENTER:
		g.drawString(lines[i].line, org_x + (width - stringwidth)/2, y);
		break;
	    case LEFT:
