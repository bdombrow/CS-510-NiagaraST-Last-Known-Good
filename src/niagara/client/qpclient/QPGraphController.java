...
/**
 * A basic implementation of GraphController, which works with simple graphs
 * that have edges connecting simple nodes. It sets up some simple interaction
 * on its view's pane.
 * 
...
public class QPGraphController extends AbstractGraphController {
	...
	public QPGraphController() {
		...
		/*
		 * ct.setFigureFilter(new Filter() { public boolean accept (Object o) {
		 * Figure f = (Figure)o return (f.getUserObject() instanceof Node) || (f
		 * instanceof FigureWrapper && ((FigureWrapper)f).getChild().instanceof
		 * Node); } });
		 */
	...
	/**
	 * Initialize all interaction on the graph pane. By the time this method is
	 * called, all relevant references to views, panes, and roles must already
	 * have been set up.
	 */
	public void initializeInteraction() {
		...
		PropertyEditorInteractor _propertyEditor = new PropertyEditorInteractor();
		_propertyEditor.setMouseFilter(new MouseFilter(3));
	...
	/**
	 * An inner class that places a node at the clicked-on point on the screen,
	 * if control-clicked with mouse button 1. This needs to be made more
	 * customizable.
	 */
	...
	protected class EdgeDropper implements ConnectorListener {
		...
		/**
		 * Called when a connector end is dropped--attach or detach the edge as
		 * appropriate.
		 * 
		 * is currently snapped to a target, the target can be obtained from the
		 * event as the source field.
		 */
	...
	protected class EdgeCreator extends AbstractInteractor {
		public void mousePressed(LayerEvent e) {
			...
			// Connector ef = view.getEdgeRenderer().render(edge, tailSite,
			// headSite);
	...
	protected class PropertyEditorInteractor extends AbstractInteractor {
		public void mousePressed(LayerEvent e) {
			...
			new PropertyEditor(null, qpn.getType() + ": "
					+ (String) qpn.getProperty("id"), qpn);
		}
	}
}
