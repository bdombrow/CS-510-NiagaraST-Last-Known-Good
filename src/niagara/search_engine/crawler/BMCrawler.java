...
/**********************************************************************
  $Id: BMCrawler.java,v 1.1 2000/05/30 21:03:27 tufte Exp $
...
**********************************************************************/
 ...
 public class BMCrawler extends Crawler {
 ...
 public BMCrawler(int threads,int sdomain) {
     super();
     goCrawlers();
  }

 public BMCrawler(String url,int threads,int sdomain) {
 ...
 protected void processPage(String url) {
     ...
     System.out.println( "Parsing ..."+url+"\n"+
			 ...
			 "\tXML Found="+BMStorageManager.cntXml()+
		       "\tPages Visited="+cntPage+
			 "\tURLs in URLQueue="+BMStorageManager.cntQ()+
			 "\tURLS in HTTable="+BMStorageManager.cntHT());
	...
	if(linkPrefix == null || 
      ...
      BMStorageManager.addUrls(sb.toString());
      in.close();
     } catch (Exception e) {
 ...
 public static void main(String args[]) throws Exception {
     ...
     String usage = " Usage: SECrawler -s url [-m domain] [-t thread] \n";
     ...
     */
	 ...
	 /* Configure the Crawler according to input specifications */
         ...
         BMStorageManager.initialize();

	 if( server != null) 
	     BMStorageManager.setSEServer(server);
	 if ( fname != null)
	     BMStorageManager.setUrlFile(fname);
	 if( ttl != -1)
	     Crawler.setTTL(ttl);
	 ...
	 if (s != null)
	  myCrawler = new BMCrawler(s,t,m);
	 else
	  myCrawler = new BMCrawler(t,m);   
