	...
	public SAXDOMReader(FileInputStream fis) {
		...
		if (BufferManager.getPageSize() > MAX_EVENTS)
			throw new PEException(
					"In-memory page size should not be larger than on-disk page size");
	}
	...
	public void readDocuments(SinkTupleStream outputStream)
		...
		while (true) {
			...
			while (off < current_offset) {
				...
				last = page.loadEvents(off, current_offset - off, types,
						string_indexes, all_strings, open_nodes, this);
				if (last == -1)
					break;
				else
					off += last - prevLast;
