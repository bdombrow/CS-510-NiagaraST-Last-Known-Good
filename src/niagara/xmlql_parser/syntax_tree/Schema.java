...
public class Schema {
   ...
   /**
    * used to generate regular expression (dots are used to join tags along a 
    * path)
    *
    * @param ith schemaunit whose path from the root element needs to be 
    *             calculated
    * @return regular expression representing the path of this element from
    *             the root element
    */
   ...
   public regExp getRegExp(int i) {
	...
	if(i==0)
		return null;
	SchemaUnit currUnit = (SchemaUnit)tupleDes.elementAt(i);
	regExp currExp = currUnit.getRegExp();
	while((j=currUnit.getBackPtr()) != 0) {
		currUnit = (SchemaUnit)tupleDes.elementAt(j);
		currExp = new regExpOpNode(opType.DOT,currUnit.getRegExp(),currExp);
	}
	return currExp;
   }
