...
import niagara.optimizer.colombia.RuleSet;
import niagara.optimizer.colombia.SSP;
import niagara.optimizer.colombia.Task;
import niagara.optimizer.colombia.Tracer;
...
public class Optimizer implements Tracer {
	...
	public Optimizer() {
		...
		consolidationRules = new RuleSet(catalog
				.getRules("consolidation ruleset"), true);
	...
	public Plan consolidate(Expr expr) {
		...
		Plan consPlan = Plan.getPlan(consolidationSSP
				.extractLastLogicalExpression(consolidationSSP.getGroup(0)),
				catalog);
	...
	public Plan optimize(Expr expr) {
		ssp.optimize(expr);
		Plan optPlan = Plan.getPlan(ssp.copyOut(ssp.getGroup(0),
				PhysicalProperty.ANY, new HashMap()), catalog);
		ssp.clear();
