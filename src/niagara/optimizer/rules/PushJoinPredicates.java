...
/**
 * Push as many of the non-equijoin predicates of a join operator to its inputs
 */
public class PushJoinPredicates extends CustomRule {
	public PushJoinPredicates(String name) {
		...
		// This is not necessarily true, we may be able
				...
				new Expr(new Join(), new Expr(new Select(), new Expr(
						new LeafOp(0)), new Expr(new Select(), new Expr(
						new LeafOp(1))))));
	}
	...
	public Expr nextSubstitute(Expr before, MExpr mexpr,
		...
		Join newJ = new Join(remJoin, eq, j.getProjectedAttrs(), j
				.getExtensionJoin(), j.getPunctAttrs());
