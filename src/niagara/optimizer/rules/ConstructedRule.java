...
/**
 * Constructed rules are rules constructed at run time from an XML specification
 * (but can have more complex before and after patterns than
 * <code>SimpleRule</code> allows).
 */
...
public class ConstructedRule extends ParsedRule {
	...
	private ConstructedRule(String name, Antecedent before, Consequent after,
			String conditionArgument, String[] maskBefore, String[] mask) {
		super(name, before.toExpr().numLeafOps(), before.toExpr(), after
				.toExpr(), maskBefore, mask);
		this.before = before;
		this.after = after;
		...
		this.conditionArgClass = before.findByName(conditionArgument)
	...
	public static ConstructedRule fromXML(Element e, Catalog catalog) {
		...
		Element beforeRoot = XMLUtils.getFirstElementChild(XMLUtils
				.getFirstElementChild(e, "before", checkPatterns), "op",
				new ConfigurationError("The root of the antecedent pattern in "
						+ name + " must be an op element"));
		Element afterRoot = XMLUtils.getFirstElementChild(XMLUtils
				.getFirstElementChild(e, "after", checkPatterns), "op",
				new ConfigurationError("The root of the consequent pattern in "
						+ name + " must be an op element"));

		Antecedent before = Antecedent.fromXML(beforeRoot, catalog);
		Consequent after = Consequent.fromXML(afterRoot, catalog);

		ConstructedRule constructedRule = new ConstructedRule(name, before,
				after, e.getAttribute("argument"), parseMask(e, "maskBefore"),
				parseMask(e, "mask"));
		constructedRule.parseCondition(e, before.getOperator().getClass(),
				catalog);
	...
	protected Object[] getConditionArgs(Expr before) {
		if (conditionArgAddress == null)
			...
			return new Object[] { this.before.followAddress(
					conditionArgAddress, before) };
	}
