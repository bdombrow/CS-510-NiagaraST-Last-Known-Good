...
public class AssociateJoinLtoR extends CustomRule {
	public AssociateJoinLtoR(String name) {
		super(name, 3, new Expr(new Join(), new Expr(new Join(), new Expr(
				new LeafOp(0)), new Expr(new LeafOp(1))), new Expr(
				new LeafOp(2))), new Expr(new Join(), new Expr(new LeafOp(0)),
				new Expr(new Join(), new Expr(new LeafOp(1)), new Expr(
						new LeafOp(2)))));
	}
	...
	public Expr nextSubstitute(Expr before, MExpr mexpr,
		...
		UpdateableEquiJoinPredicateList newPreds1 = new UpdateableEquiJoinPredicateList();
		UpdateableEquiJoinPredicateList newPreds2 = new UpdateableEquiJoinPredicateList();
		...
		// allowed
		if (!ruleSet.allowCartesian() && !op1.isCartesian()
				&& !op2.isCartesian()
		...
		Predicate abcPred = And.conjunction(split2.getRight(), op1
				.getNonEquiJoinPredicate());
		Join newOp2 = new Join(abcPred, newPreds2, Join.NONE);

		// Phew!
		return new Expr(newOp2, new Expr(A), new Expr(newOp1, new Expr(B),
				new Expr(C)));
	}
