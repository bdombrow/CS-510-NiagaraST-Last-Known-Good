...
/**
 * Consequent patterns for <code>ConstructedRule</code>s. A node in a consequent
 * pattern can be a simple copy of an operator in the antecedent, or a new
 * operator, optionally initialized with one or more operators from the
 * antecedent.
 */
...
public class Consequent extends Pattern {
	...
	// A sequence of numbers that identify the position of <code>name</code> in
	// the antecedent pattern.
	private byte[] address;
	// Addresses of the initializers
	private byte[][] extraAddresses;

	private Consequent(String name, String[] initializers, Op operator,
			Consequent[] inputs) {
	...
	public static Consequent fromXML(Element e, Catalog catalog) {
		...
		if (e.getTagName().equals("leaf")) {
			...
			if (posStr.length() == 0)
				Catalog
						.confError("Leaves must be annotated with a name attribute");

			return new Consequent(posStr, new String[] {}, null,
					new Consequent[] {});
		}
		...
		if (!((name != null) ^ (opName.length() != 0)))
			Catalog
					.confError("Must provide either a pattern or an operator name (but not both)");
		...
		if (opName.length() != 0) {
			...
			try {
				...
				throw new PEException("Constructor of " + opClass
						+ " has thrown an exception: "
						+ ite.getTargetException());
			} catch (IllegalAccessException iae) {
				throw new PEException("An illegal access exception occured: "
						+ iae);
			} catch (InstantiationException ie) {
				throw new PEException("An instantiation exception occured: "
						+ ie);
			}
		}
	...
	public Expr constructSubstitute(Expr before) {
		...
		for (int i = 0; i < extraAddresses.length; i++)
			((Initializable) newOp).initFrom((LogicalOp) followAddress(
					extraAddresses[i], before));
