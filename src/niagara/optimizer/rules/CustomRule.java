...
abstract public class CustomRule extends Rule {
	...
	/*
	 * If this rule fires, disallow firing any rule whose name appears in
	 * maskBefore on the "before" expression
	 */
	protected String[] maskBeforeRuleNames;
	/*
	 * If this rule fires, disallow firing any rule whose name appears in mask
	 * on the "after" expression
	 */
	...
	public static CustomRule fromXML(Element e, Catalog catalog) {
		...
		try {
			...
			// (the rule name)
			Constructor ruleConstructor = c
					.getConstructor(new Class[] { String.class });
			rule = (CustomRule) ruleConstructor
			...
			throw new PEException("Constructor of " + className
