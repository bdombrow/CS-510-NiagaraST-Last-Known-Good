...
public class AssociateJoinRtoL extends CustomRule {
	...
	public AssociateJoinRtoL(String name) {
		super(name, 3, new Expr(new Join(), new Expr(new LeafOp(0)), new Expr(
				new Join(), new Expr(new LeafOp(1)), new Expr(new LeafOp(2)))),
				new Expr(new Join(), new Expr(new Join(), new Expr(
	...
	public Expr nextSubstitute(Expr before, MExpr mexpr,
		...
		// Phew!
		return new Expr(newOp2, new Expr(newOp1, new Expr(A), new Expr(B)),
				new Expr(C));
	}
