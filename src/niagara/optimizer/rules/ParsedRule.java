...
public abstract class ParsedRule extends Rule {
	...
	/**
	 * A simple boolean method an operator class that takes no arguments and
	 * checks that the rule is applicable
	 */
	...
	protected void parseCondition(Element e, Class c, Catalog catalog) {
		conditionMethod = null;
		String condition = e.getAttribute("condition");
		if (condition.length() != 0) {
			try {
				conditionMethod = c
						.getMethod(condition, getConditionArgTypes());
	...
	/*
	 * If this rule fires, disallow firing these rules on the "before"
	 * expression
	 */
	...
	protected ParsedRule(String name, int arity, Expr before, Expr after,
	...
	/**
	 * @see niagara.optimizer.colombia.Rule#condition(Expr, MExpr,
	 *      PhysicalProperty)
	 */
	public boolean condition(Expr before, MExpr mexpr, PhysicalProperty ReqdProp) {
		if (conditionMethod == null)
			return true;
		try {
			return ((Boolean) conditionMethod.invoke(before.getOp(),
					getConditionArgs(before))).booleanValue();
