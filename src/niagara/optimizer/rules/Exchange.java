...
public class Exchange extends CustomRule {
	public Exchange(String name) {
		super(name, 4, new Expr(new Join(), new Expr(new Join(), new Expr(
				new LeafOp(0)), new Expr(new LeafOp(1))), new Expr(new Join(),
				new Expr(new LeafOp(2)), new Expr(new LeafOp(3)))), new Expr(
				new Join(), new Expr(new Join(), new Expr(new LeafOp(0)),
						new Expr(new LeafOp(2))), new Expr(new Join(),
						new Expr(new LeafOp(1)), new Expr(new LeafOp(3)))));
	}
	...
	public Expr nextSubstitute(Expr before, MExpr mexpr,
		...
		// find an equivalent attribute we push it to one of the lower joins as
		// well
		//forallpreds: 
		...
		// Check that we're not producing a cartesian product if it's not
		// allowed
		if (!ruleSet.allowCartesian()
				&& !abJoin.isCartesian()
				&& !cdJoin.isCartesian()
				&& !abcdJoin.isCartesian()
				&& (acPreds.isEmpty() || bdPreds.isEmpty() || acbdPreds
						.isEmpty()))
			return null;
		...
		Predicate allNonEquiPreds = And.conjunction(abcdJoin
				.getNonEquiJoinPredicate(), And.conjunction(abJoin
				.getNonEquiJoinPredicate(), cdJoin.getNonEquiJoinPredicate()));
		...
		return new Expr(acbdJoin, new Expr(acJoin, new Expr(A), new Expr(C)),
	...
	public boolean condition(Expr before, MExpr mexpr, PhysicalProperty ReqdProp) {
		...
		// If we do not allow a non-Cartesian product to go to a Cartesian
		// product
		// Need to check:
	...
	/**
	 * Find an attribute in <code>schema</code> (if any) that belongs in a given
	 * equivalence class
	 */
