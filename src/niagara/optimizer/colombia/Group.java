...
/***
 * The main problem, and its subproblems, consist of a search to find the
 * cheapest MultiExpression in a Group, satisfying some context.
 * 
 * A Group includes a collection of logically equivalent MExpr's. The Group also
 * contains logical properties shared by all MExpr's in the group.
 * 
 * A Group also contains a winner's circle consisting of winners from previous
 * searches of the Group. Note that each search may of the Group may have
 * different contexts and thus different winners.
 * 
 * A Group can also be thought of as a temporary collection, or subquery, in a
 * computation of the main query.
 * 
 * We assume the following three conditions are equivalent: (1) There is a
 * winner (maybe with null *plan) for some property, with done==true
 * 
 * (2) The group contains all possible logical and physical expressions except
 * for enforcers
 * 
 * (3) The bit "optimized" is turned on.
 * 
 * And any of these imply: (4) For any property P, the cheapest plan for
 * property P is either in G or is an enforcer for P
 * 
 * The truth of the above assumptions depend on whether we fire all applicable
 * rules when the property is ANY.
 */
...
public class Group {
	...
	public Group(MExpr mexpr, SSP ssp) {
		...
		assert mexpr.getOp().isLogical() : "Group initialized by physical multiexpression";
		...
		/*
		 * Calculate the LowerBd, which is: TouchCopyCost: touchcopy() * |G| +
		 * //From top join // from other, nontop, joins touchcopy() *
		 * sum(cucard(Ai) i = 2, ..., n-1) + + FetchCost: fetchbound() *
		 * sum(cucard(Ai) i = 1, ..., n) // from leaf fetches
		 */
		double cost = 0;
	...
	/*
	 * boolean search_circle(Context * C, boolean & moresearch) { First search
	 * for a winner with property P. If there is no such winner, case (3) If
	 * there is a winner, denote its plan component by WPlan and its cost
	 * component by WCost. Context cost component is CCost
	 * 
	 * If (WPlan is non-null) //Cheapest plan costs *WCost; //we seek a plan
	 * costing *CCost or less If (*WCost <= *CCost) Case (2) else if (*CCost <
	 * *WCost) Case (1) else If (WPlan is null) //All plans cost more than
	 * *WCost if(*CCost <= *WCost) Case (1) else if (*WCost < *CCost) //There
	 * might be a plan between WCost and CCost Case (4)
	 */

	/*
	 * search_circle returns the state of the winner's circle for this context
	 * and group - it does no rule firing. Thus it is cheap to execute.
	 * search_circle returns in four possible states: (1) There is no
	 * possibility of satisfying C (2) There is a non-null winner for C (3) More
	 * search is needed, and there has been no search for this property before
	 * (4) More search is needed, and there has been a search for this property
	 * before. See the pseudocode for search_circle (in *.cpp) for how these
	 * states arise.
	 * 
	 * search_circle could adjust the current context, but won't because: It is
	 * not necessary for correctness It is likely not very useful
	 */
	public static class SearchResults {
		...
		public static final SearchResults CONTINUING_SEARCH = new SearchResults();
	...
	public SearchResults search_circle(Context c) {
		...
		if (m != null) {
			if (cCost.greaterThanEqual(wCost)) // there is a non-null winner
				...
				// search is impossible as winner's cost is more than
	...
	void newWinner(PhysicalProperty reqdProp, MExpr mexpr, Cost totalCost,
			boolean done) {
		changed = true;
		...
		for (int i = winners.size(); --i >= 0;) {
			PhysicalProperty winPhysProp = ((Winner) winners.get(i))
					.getPhysProp();
	 ...
	 * TouchCopyCost = TouchCopy() * |G| + //From top join TouchCopy() *
	 * sum(cucard(Ai) i = 2, ..., n-1) + // from other, nontop,
	 */// joins
	// TouchCopyCost are not supported as my model does not include base
	// collection
	...
	double touchCopyCost(LogicalProperty logProp) {
		...
		/*
		 * // from A2 -- An-1 , means excluding the min and max cucard(i) double
		 * Min= 3.4E+38; double Max=0; for(int i=0; i <
		 * LogProp->Schema.GetTableNum(); i++) { //float CuCard =
		 * LogProp->Schema.GetTableMaxCuCard(i); float CuCard =
		 * LogProp.(*Attrs)[i]->CuCard; if(Min > CuCard) Min = CuCard; if(Max <
		 * CuCard) Max = CuCard; Total += CuCard ; }
		 * 
		 * // exclude min and max Total -= Min; Total -= Max;
		 */
	...
	// os += PhysProp -> Dump();
	// os += ", " ;
	// os += (Winners[i].GetMPlan() ? Winners[i].GetMPlan()-> Dump() :
	// "null Plan");
	// os += ", " ;
	...
	/*
	 * for cucard pruning, calculate the minimun cost of fetching cucard tuples
	 * from disc FetchingCost = Fetch() * sum(cucard(Ai) i = 1, ..., n) // from
	 * leaf fetches For each Ai which has no index on a join (interesting)
	 * order, replace cucard(Ai) with |Ai| and it is still a lower bound.
	 */

	// FetchingCost are not currently supported as my model does not include
	// base collection
	// information for intermediate, which, however, can be accomplished.
	// Now, return only the cost of fetching visible attribtes at the current
	// point.
	// Quan
	// 10/99
	...
	double fetchingCost(LogicalProperty logProp) {
		...
		/*
		 * for(int i=0; i < LogProp-> Schema.GetTableNum(); i++) { float CuCard
		 * = LogProp->Schema.GetTableMaxCuCard(i); float Width =
		 * LogProp->Schema.GetTableWidth(i); Total += ceil(CuCard * Width) * (
		 * // cpu cost of reading from disk Cm.io()); // i/o cost of reading
		 * from disk }
		 */
