...
public class Bindery {
	...
	Expr extract_expr() {
		...
		else // general invocation of new Expr
		{
			...
			// Need the arity of the top operator to construct inputs of new
			// Expr
	...
	/*
	 * Function advance() walks the many trees embedded in the MEMO structure in
	 * order to find possible bindings. It is called only by ApplyRule::perform.
	 * The walking is done with a finite state machine, as follows.
	 * 
	 * State start: If the pattern is a leaf, we are done. State = finished
	 * Return TRUE Skip over non-logical, non-matching expressions. State =
	 * finished break Create a group bindery for each input and try to create a
	 * binding for each input. If successful State = valid_binding Return TRUE
	 * else delete input binderys State = finished break
	 * 
	 * 
	 * State valid_binding: Increment input bindings in right-to-left order. If
	 * we found a next binding, State = valid_binding return TRUE else delete
	 * input binderys state = finished break
	 * 
	 * 
	 * State finished If pattern is a leaf //second time through, so we are done
	 * OR this is an expr bindery //we finished the first expression, so done OR
	 * there is no next expression return FALSE else state = start break
	 */
	boolean advance() {
		if (ssp.REUSE_SIB) {
			Op patt_op = pattern.getOp();
			...
			if (!one_expr && state == BINDERY_STATE.START) // begin the group
															// binding
			...
			for (;;) {
				...
				if (state == BINDERY_STATE.START) {
					...
					if (!patt_op.matches(op_arg)) {
						...
						return true;
					} else { // successful bindings for the Operator without
								// inputs
						// Create a group bindery for each input
						input = new Bindery[arity];
						for (input_no = 0; input_no < arity; input_no++) {
							input[input_no] = new Bindery(cur_expr
									.getInput(input_no), pattern
									.getInput(input_no), ssp);
				...
				/*
				 * // try existing inputs in right-to-left order // first
				 * success is overall success
				 * 
				 * for (input_no = arity; -- input_no >= 0; ) { if (
				 * input[input_no].advance() ) // found one more binding { // If
				 * we have a new binding in a non-rightmost location, // we must
				 * create new binderys for all inputs to the // right of
				 * input_no, else we will not get all bindings. // This is
				 * inefficient code since the each input on the // right has
				 * multiple binderys created for it, and each // bindery
				 * produces the same bindings as the others. // The simplest
				 * example of this is the exchange rule. for (int other_input_no
				 * = input_no; ++ other_input_no < arity; ) { delete
				 * input[other_input_no]; input[other_input_no] = (new Bindery (
				 * cur_expr.GetInput(other_input_no), pattern
				 * .GetInput(other_input_no)) );
				 * 
				 * if (! input[other_input_no].advance() ) // Impossible since
				 * we found these bindings earlier ASSERT(false) ; }
				 * 
				 * // return overall success state = valid_binding; return true;
				 * } // found one more binding } // try existing inputs in
				 * right-to-left order
				 * 
				 * // There are no more bindings to this log expr; // dealloc
				 * input binderys. if(arity) { for (input_no = arity; --
				 * input_no >= 0; ) delete input[input_no]; delete [] input;
				 * input = null; } state = finished; break;
				 */
				else if (state == BINDERY_STATE.FINISHED) {
					if (one_expr
			...
			for (;;) {
				...
				if (state == BINDERY_STATE.START) {
					...
					for (input_no = arity; --input_no >= 0;) {
						...
						{
							// If we have a new binding in a non-rightmost
							// location,
							// we must create new binderys for all inputs to the
							// right of input_no, else we will not get all
							// bindings.
							// This is inefficient code since the each input on
							// the
							// right has multiple binderys created for it, and
							// each
							// bindery produces the same bindings as the others.
							// The simplest example of this is the exchange
							// rule.
							for (int other_input_no = input_no; ++other_input_no < arity;) {
								input[other_input_no] = (new Bindery(cur_expr
										.getInput(other_input_no), pattern
										.getInput(other_input_no), ssp));

								if (!input[other_input_no].advance())
									// Impossible since we found these bindings
									// earlier
									assert false;
							}
	...
	private void newBindery(int arity) {
		...
		for (int i = 0; i < arity; i++)
			input[i] = new Bindery(cur_expr.getInput(i), pattern.getInput(i),
					ssp);
	}
}
