 ...
 * problem In query optimization the problem is to find the cheapest plan for a
 * given query subject to certain context (class Context).
 * 
 * A Search Space typically consists of a collection of possible solutions to
 * the problem and its subproblems. Dynamic Programming and Memoization are two
 * approaches to using a Search Space to solve a problem. Both Dynamic
 * Programming and Memoization partition the possible solutions by logical
 * equivalence. We will call each partition a Group. Thus a Group contains a
 * collection of logically equivalent expressions.
 * 
 * In our setting of query optimization, logical equivalence is query
 * equivalence. Each Group corresponds to a temporary collection, or a subquery,
 * in a computation of the main query. If the main query is A join B join C,
 * then there are GROUPs representing A join B, A join C, C, etc.
 * 
 * In our approach to query optimization, each possible solution in the Search
 * Space is represented compactly, by what we call a Multi-expression (class
 * MExpr). A solution in the search space can also be represented in more detail
 * by an expression (class Expr).
 */
...
public class SSP {
	...
	// _COSTS_ Prints the cost of each mexpr as it is costed, in the output
	// window
	// _TABLE_: prints one summary line for each optimization, using different
	// epsilons
	// _GEN_LOG: Used to control the generation of logical expressions when eps
	// pruning is done.
	public boolean GEN_LOG = false;

	// REUSE_SIB: An attempt to improve pattern matching by reusing one side of
	// generated mexprs.
	public boolean REUSE_SIB = false;

	// INFBOUND: When optimizing a group, ignore the initial upper bound; use
	// infinity instead
	...
	// std::vector<Expr*> * AllExprs(int GrpID, PhysicalProperty * PhysProp,
	// boolean forLogical);
	...
	// if(! Group.is_optimized() && ! Group.is_explored()) return ; // may be
	// pruned

	// PTRACE("Shrinking group %d,", group_no);

	// // Shrink the logical mexpr
	// // init the rule mark of the first mexpr to 0, means all rules are
	// allowed
	...
	/**
	 * Return registered multiexpression that is equal (but
	 * <em>not identical</em>) to <code>MExpr</code>, register
	 * <code>MExpr</code> and return null if there is none.
	 */
	...
	public MExpr copyIn(Expr Expr, int GrpID, boolean returnDuplicate) {
		...
		if (MExpr.getOp().isLogical()) {
			...
			if (DupMExpr != null) { // not null ,there is a duplicate
				...
				// If the Mexpr is supposed to be in a new group, set the group
				// id
				if (GrpID == NEW_GRPID) {
	...
	/**
	 * Return the optimal expression that satisfies this property, null
	 * otherwise
	 */
		...
		if (group.getFirstLogMExpr().getOp().is_item()) {
			...
			for (int i = 0; i < winnerMExpr.getArity(); i++) {
				...
				if (seen.containsKey(inputGroup))
					...
					inputs[i] = copyOut(winnerMExpr.getInput(i),
							PhysicalProperty.ANY, seen);
			}
			...
			for (int i = 0; i < arity; i++) {
				...
				PhysicalProperty[] properties = ((PhysicalOp) winnerOp)
						.inputReqdProp(physProp, inputGroup.getLogProp(), i);
	...
	public void optimize(Expr expr) {
		...
		// Create initial context, with no requested properties, infinite upper
		// bound,
		// zero lower bound, not yet done. Later this may be specified by user.
		InitCont = new Context(new PhysicalProperty(Order.newAny()), new Cost(
				-1), false);
		...
		if (GlobepsPruning) {
			...
			ptasks.push(new O_GROUP(this, (Group) Groups.get(RootGID),
					InitCont, true, eps_bound));
		} else
			ptasks
					.push(new O_GROUP(this, (Group) Groups.get(RootGID),
							InitCont));
