...
public class O_INPUTS extends Task {
	...
	private int arity;
	private int inputNo; // input currently being or about to be optimized,
							// initially 0
	 ...
	 * perform
	 * 
	 * NOTATION InputCost[]: Contains actual (or lower bound) costs of optimal
	 * inputs to G. CostSoFar: LocalCost + sum of all InputCost entries. G: the
	 * group being optimized. IG: various inputs to expressions in G. SLB:
	 * (Special Lower Bound) The Lower Bound of G, derived with fetch and cucard
	 * We use this term instead of Lower Bound since we will use other lower
	 * bounds.
	 * 
	 * There are still three flags: Pruning (also called Group Pruning),
	 * CuCardPruning and GlobepsPruning, with new meanings. We plan to run
	 * benchmarks in four cases:
	 * 
	 * 1. Starburst - generate all expressions [!Pruning && !CuCardPruning] 2.
	 * Group Pruning - aggressively check limits at all times [Pruning &&
	 * !CuCardPruning] aggressively check means if (CostSoFar >= upper bound of
	 * context in G) then terminate. 3. Lower Bound Pruning - if there is no
	 * winner, then use IG's SLB in InputCost[]. [CuCardPruning]. This case
	 * assumes that the Pruning flag is on, i.e. the code forces Pruning to be
	 * true if CuCardPruning is true. The SLB may involve cucard, fetch, copy,
	 * etc in the lower bound. 4. Global Epsilon Pruning [GlobepsPruning]. If a
	 * plan costs <= GLOBAL_EPS, it is a winner for G.
	 * 
	 * PSEUDOCODE
	 * 
	 * On the first (and no other) execution, the code must initialize some
	 * O_INPUTS members. The idea here is to get a quick lower bound for the
	 * cost of the inputs. The only nontrivial member is InputCost; here is how
	 * to initialize it: //Initial values of InputCost are zero in the Starburst
	 * case For each input group IG If (Starburst case) InputCost is zero
	 * continue Determine property required of search in IG If no such property,
	 * terminate this task. call search_circle on IG with that property,
	 * infinite cost.
	 * 
	 * If case (1), no possibility of satisfying the context terminate this task
	 * If search_circle returns a non-null Winner from IG, case (2)
	 * InputCost[IG] = cost of that winner else if (!CuCardPruning) //Group
	 * Pruning case (since Starburst not relevant here) InputCost[IG] = 0
	 * //remainder is Lower Bound Pruning case else if there has been no
	 * previous search for ReqdProp InputCost[IG] = SLB else if there has been a
	 * previous search for ReqdProp InputCost[IG] = max(cost of winner, IG's
	 * SLB) //This is a lower bound for IG else error - previous cost failed
	 * because of property
	 * 
	 ...
	 * 
	 * if (arity==0 and required property can not be satisfied) terminate this
	 * task
	 * 
	 * //Calculate cost of remaining inputs For each remaining (from InputNo to
	 * arity) input group IG Call search_circle() If Starburst case and case (1)
	 * error else if case (1) terminate this task else If there is a non-null
	 * Winner in IG, case (2) store its cost in InputCost if (Pruning &&
	 * CostSoFar exceeds G's context's upper bound) terminate task else if (we
	 * did not just return from O_GROUP on IG) //optimize this input; seek a
	 * winner for it push this task push O_GROUP for IG, using current context's
	 * cost minus CostSoFar plus InputCost[InputNo] terminate this task else //
	 * we just returned from O_GROUP on IG Trace: This is an impossible plan
	 * terminate this task InputNo++; endFor //calculate the cost of remaining
	 * inputs
	 * 
	 * //Now all inputs have been optimized
	 * 
	 * if (CostSoFar > G's context's upper bound) terminate this task
	 * 
	 * //Now we know current expression satisfies current context.
	 * 
	 * if(GlobepsPruning && CostSoFar <= GLOBAL_EPS) Make current mexpression a
	 * done winner for G mark the current context as done terminate this task
	 * 
	 ...
	 * If there is no such winner error - the search should have initialized a
	 * winner else If winner is done error - we are in the midst of a search,
	 * not yet done else If (winner is non-null and CostSoFar >= cost of this
	 * winner) || winner is null Replace existing winner with current
	 * mexpression and its cost, don't change done flag Update the upper bound
	...
	public void perform() {
		...
		assert mexpr.getOp().isPhysical() : "We can only optimize the inputs of physical expressions";
		...
		if (ssp.GlobepsPruning && context.isFinished()) {
			...
			if (last)
				// this's the last task for the group, so mark the group with
		...
		// On the first (and no other) execution, code must initialize some
		// O_INPUTS members.
		...
		if (inputNo == -1) {
			...
			localCost = Op.findLocalCost(ssp.getCatalog(), inputLogProp);

			// For each input group IG
			for (input = 0; input < arity; input++) {
				// Initial values of InputCost are zero in the Starburst (no
				// Pruning) case
				if (!ssp.Pruning) {
					if (input == 0)
						// PTRACE(
				...
				if (Op.isPhysical()) {
					...
					properties = ((PhysicalOp) Op).inputReqdProp(LocalReqdProp,
							inputLogProp[input], input);
					...
					if (properties == null) {
						...
						if (properties.length == 0) {
							...
							// allows an operator to specify that any of a
							// number
				...
				// If search_circle returns a non-null Winner from InputGroup,
				// case (2)
				...
				// else if (!CuCardPruning) //Group Pruning case (since
				// Starburst not relevant here)
				// InputCost[IG] = 0
			...
			inputNo++;
			// Ensure that previous code will not be executed again; begin with
			// Input 0
		}

		// If Global Pruning and cost so far is greater than upper bound for
		// this context, then terminate
		...
		for (input = inputNo; input < arity; input++) {
			...
			// else if Case 2: There is a winner with nonzero plan, in current
			// input
			else if (sr.haveWinner()) {
			...
			// Remaining cases are (3) and (4)
				...
				// Adjust PrevInputNo and InputNo to track progress after
				// returning from pushes
				prevInputNo = input;
				inputNo = input;
				...
				if (ssp.GlobepsPruning) {
					...
					if (epsBound.greaterThan(localCost)) {
						...
						// calculate the cost, the lower nodes should have lower
						// eps bound
					...
					ssp.addTask(new O_GROUP(ssp, mexpr.getInput(input),
							inputContext, true, eps_bound));
				} else
					ssp.addTask(new O_GROUP(ssp, mexpr.getInput(input),
							inputContext));
		...
		if (arity == 0 && !LocalReqdProp.getOrder().isAny() && Op.isPhysical()) {
			PhysicalProperty OutputPhysProp = ((PhysicalOp) Op)
					.findPhysProp(PhysicalOp.NO_INPUTS);
		...
		// All inputs have been been optimized, so compute cost of the
		// expression being optimized.
		...
		// if halt, halt optimize the group when either number of plans since
		// the
		// last winner >= HaltGrpSize*EstiGrpSize or the improvement in last
		// HaltWinSize
		...
		if (LocalWinner.getMPlan() != null && // If there is already a non-null
												// local winner
				CostSoFar.greaterThanEqual(LocalWinner.getCost()) // and current
																	// expression
																	// is more
																	// expensive
		) {
	...
	void terminateThisTask(Winner LocalWinner) {
		...
		if (ssp.NO_PHYS_IN_GROUP)
			// delete this physical mexpr to save memory; it may not be used
			// again.
			mexpr = null;
