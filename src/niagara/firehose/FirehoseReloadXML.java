...
class RefreshXML extends Thread {
	...
	public void run() {
		...
		try {
			...
			String stURL = new String(
					"http://church.cse.ogi.edu:8088/FILE?REG="
							+ String.valueOf(ss.getLocalPort()));

			m_aptContainer.getAppletContext().showDocument(new URL(stURL),
					"xml");
			...
			while (true) {
				...
				m_aptContainer.getAppletContext().showStatus(
						"Waiting for input...");
				is.read(rgbMsg, 0, 4);

				if (rgbMsg[0] == 1 && rgbMsg[1] == 0 && rgbMsg[2] == 0
						&& rgbMsg[3] == 0) {
					m_aptContainer.getAppletContext()
							.showStatus("Reloading...");
					// update the xml frame with new data
					m_aptContainer.getAppletContext().showDocument(
							new URL("http://church.cse.ogi.edu:8088/FILE" + i),
							"xml");
