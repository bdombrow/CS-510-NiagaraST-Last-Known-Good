...
class XMLAuctionStreamGenerator extends XMLFirehoseGen {
	...
	// will generate bids, items and persons in a ratio of 10 bids/item 5
	// items/person
	...
	private PersonGen p = new PersonGen(); // used for generating values for
											// person
	...
	public XMLAuctionStreamGenerator(int numTLElts, int numGenCalls,
			boolean streaming, boolean prettyPrint, BufferedWriter wrtTrace) {
		// numTLElts is ignored for now, but leave it in case I change my mind -
		// KT
	...
	private void generateBid(MyBuffer myb, int numBids) throws IOException {
		...
		for (int i = 0; i < numBids; i++) {
			...
			myb.append(System.currentTimeMillis()
					- rnd.nextInt(MAXINCREMENT_MILLISEC));
			myb.append("</time>");
			myb.append(nl);
		...
		for (int i = 0; i < numItems; i++) {
			...
			if (!LIMIT_ATTRIBUTES) {
				...
				if (rnd.nextBoolean()) {
					myb.append(tab2);
					myb.append("<reserve>");
					// was generated using an exponential distribution, but I
					// changed
					// it to uniform KT
					// HERE - TODO
					myb.append((int) Math.round((openAuctions
							.getCurrPrice(auctionId))
			...
			// assume itemId and openAuctionId are same - only one auction per
			// item allowed
			myb.append(auctionId);
			myb.append("\"></itemref>");
			myb.append(nl);
			...
			// skip annotation - too hard to generate - need to just get this
			// done KT
	...
	// Item contains:
	// location, quantity, name, payment, description, shipping, incategory+,
	// mailbox)>
	// weird, item doesn't contain a reference to the seller, open_auction
	// contains
