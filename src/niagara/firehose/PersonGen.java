...
class PersonGen {
	...
	public void generateValues(OpenAuctions auctions) {
		...
		has_watches = false;
		/*
		 * skip watches for now - expensive and problem with people who are
		 * generated before any items if (m_rnd.nextBoolean()) { int cWatches =
		 * m_rnd.nextInt(20) + 1; int iWatch; for (int i=0; i<cWatches; i++) {
		 * // is this OK, will this screw up bids/items distribution??
		 * m_vctWatches.add(String.valueOf(auctions.getExistingId())); } } else
		 * { m_vctWatches.clear(); }
		 */
	}
	...
	private void genAddress() {
		...
		int icn = (m_rnd.nextInt(4) != 0) ? 0 : m_rnd
				.nextInt(BigValueGen.NUM_COUNTRIES);
		int ipv = (icn == 0) ? m_rnd.nextInt(BigValueGen.NUM_PROVINCES) : m_rnd
				.nextInt(LastnamesGen.NUM_LASTNAMES); // provinces are really
														// states
	...
	private void genProfile() {
		if (m_rnd.nextBoolean()) {
			...
			m_profile.m_stEducation = BigValueGen.education[m_rnd
					.nextInt(BigValueGen.NUM_EDUCATION)];
		} else {
		...
		m_profile.m_stIncome
				.put(String.valueOf((m_rnd.nextInt(30000) + 40000)));
		...
		for (int i = 0; i < cInterest; i++) {
			// HERE
			m_profile.m_vctInterest.add(String.valueOf(m_rnd
					.nextInt(NUM_CATEGORIES)));
		}
	}
}
