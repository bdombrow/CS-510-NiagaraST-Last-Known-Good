...
class XMLTempGenerator extends XMLFirehoseGen {
	...
	public XMLTempGenerator(String fileName, boolean streaming,
			boolean prettyprint) {
		m_iTempCurr = 70;
		m_rnd = new Random();
		m_iTime = 0;
	...
	public String generateXMLString() {
		...
		stbRet.append("<TEMPDATA>" + "<TEMPERATURE>" + "<ID>" + m_stId
				+ "</ID>" + "<HOUR>" + String.valueOf(cHour) + "<MINUTE>"
				+ String.valueOf(cMinute) + "</MINUTE>" + "<CURRTEMP>"
				+ String.valueOf(m_iTempCurr) + "</CURRTEMP>" + "</HOUR>"
				+ "</TEMPERATURE>");
		...
		if (m_cHourLast != cHour) {
			if (m_cHourLast != -1) {
				stbRet.append("\n<punct:TEMPERATURE>" + "<ID>*</ID>" + "<HOUR>"
						+ m_cHourLast + "<MINUTE>*</MINUTE>"
						+ "<CURRTEMP>*</CURRTEMP>" + "</HOUR>"
						+ "</punct:TEMPERATURE>");
			}
			m_cHourLast = cHour;
		} else if (cMinute % 1 == 0) {
			stbRet.append("\n<punct:TEMPERATURE>");
			stbRet.append("<ID>*</ID>");
			stbRet.append("<HOUR>" + cHour + "." + "." + cMinute + "");
