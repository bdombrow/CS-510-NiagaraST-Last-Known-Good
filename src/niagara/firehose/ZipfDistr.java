...
// Maintains a Zipfian distribution and can return a random integer
// drawn from the distribution.
// Zipfian distribution is freq(i) ~ 1/i^exp, where exp close to 1
// Strictly sum(i=1 to N) Pi = numItems * avgNumEventsPerItem
// Pi is frequency and formally Pi = sum(i=1 to N) 1/i^ex
...
class ZipfDistr {
	...
	while(!found) {
	    ...
	    itemNum = rnd.nextInt(numItems); // exclusive of numItems, inclusive 0
	    double rndInt = rnd.nextDouble()*normalizer; // normalize double value
	...
	for(int i = 0; i<numItems; i++) {
	    probIN[i] = (1/Math.pow(i+1, exp))*C; // probability of itemNum
