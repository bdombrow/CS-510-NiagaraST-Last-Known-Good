...
public class XMLQueryPlanParser {
	...
	public Plan parse(String description) throws InvalidPlanException {
		...
		try {
			parser.parse(new InputSource(new ByteArrayInputStream(description
					.getBytes())));
			if (parser.hasErrors() || parser.hasWarnings()) {
				String msg = parser.getErrorStrings()
						+ parser.getWarningStrings();
	...
	Plan parsePlan(Element root) throws InvalidPlanException {
		...
		for (int i = 0; i < children.getLength(); i++) {
			...
			if (id.length() == 0)
				throw new InvalidPlanException(
						"Each operator must have an id attribute");
			if (ids2els.get(id) != null)
				throw new InvalidPlanException("Duplicate operator id found: "
						+ id);
		...
		Attrs outputSchema = ((LogicalProperty) ids2logprops.get(top))
				.getAttrs();
	...
	/**
	 * @param e
	 *            an <code>Element</code> describing a logical plan operator
	 * @exception InvalidPlanException
	 *                the description of the plan was invalid
	 */
	void parseOperator(Element e) throws InvalidPlanException {
		...
		for (int i = 0; i < inputs.size(); i++) {
			...
			if (el == null)
				throw new InvalidPlanException("Could not find input "
						+ inputs.get(i) + " of operator " + id);
			parseOperator(el);
		}
		...
		try {
			...
			throw new InvalidPlanException(nodeName
					+ " is not a logical operator");
		} catch (InstantiationException ie) {
			throw new InvalidPlanException("Could not instantiate an "
					+ opClass + " object for " + nodeName);
		} catch (IllegalAccessException iae) {
			throw new InvalidPlanException(
					"Illegal access exception while instantiating " + opClass
							+ " for " + nodeName);
		}
		int arity = inputs.size();
		LogicalProperty[] inputProperties = new LogicalProperty[arity];
		for (int i = 0; i < arity; i++)
			inputProperties[i] = (LogicalProperty) ids2logprops.get(inputs
					.get(i));
