    ...
    public static void init(String[] args) {
        ...
        if (args.length == 1 && args[0].equals("-init")) {
            ...
            try {
                ...
                while (!validInput) {
                    ...
                    validInput =
                        hostIsValid(new String(input, 0, inputLen - 1));
                ...
                while (!validInput) {
                    System.out.print(
                        "Enter the number of query threads (default 10): ");
                    inputLen = System.in.read(input);
                    if (inputLen == 0) {
                    ...
                    if (input[0] == '\n') {
                        System.out.println(
                            "Setting number of query threads to 10");
                        validInput = true;
                        input = "10".getBytes();
                        inputLen = 3;
                    } else
                        validInput =
                            isInteger(new String(input, 0, inputLen - 1));
                ...
                while (!validInput) {
                    System.out.print(
                        "Enter the number of operator threads (default 50): ");
                    inputLen = System.in.read(input);
                    if (input[0] == '\n') {
                        System.out.println(
                            "Setting number of operator threads to 50");
                        validInput = true;
                        input = "50".getBytes();
                        inputLen = 3;
                    } else
        ...
        if (configInfoFile.exists()) {
            ...
            try {
                ...
                throw new UnrecoverableException(
                    "Unable to retrieve config file " + e.getMessage());
            } catch (IOException ioe) {
                throw new UnrecoverableException(
                    "Unable to read config file " + ioe.getMessage());
            }
            return;
        }
