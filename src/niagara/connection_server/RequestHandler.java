...
/**
 * There is one request handler per client and receives all the requests from
 * that client Then that request is further dispatched to the appropriate module
...
public class RequestHandler {
	...
	// Every query is given a server query id. This is the counter for giving
	// out that service id
	...
	/**
	 * Do we need to send each response message as a separate XML document, with
	 * its own header?
	 */
	 ...
	 * @param server
	 *            The server that has access to other modules
	 */
	public RequestHandler(Socket sock, NiagraServer server) throws IOException {
		this.outputWriter = new BufferedWriter(new OutputStreamWriter(sock
				.getOutputStream()));
		this.server = server;
	...
	public RequestHandler(String queryString, String requestType,
			HttpServletResponse res, NiagraServer server) throws IOException {
		this.outputWriter = new BufferedWriter(new OutputStreamWriter(res
				.getOutputStream()));
		this.server = server;
	...
	public void handleRequest(RequestMessage request) {
		...
		try {
			...
			case KILL_PREPARED_QUERY:
				...
				if (query != null) {
					...
					optimizedPlan = new Plan(
							new ConstantOpThread("<kill_prepared id='" + planid
									+ "'/>", new Attrs()));
			...
			case RUN_GC:
				...
				System.out.println("Garbage Collection Completed." + " Time: "
						+ executetime + " seconds.");
			...
			case DUMPDATA:
				if (NiagraServer.RUNNING_NIPROF) {
					...
					ResponseMessage doneDumpMesg = new ResponseMessage(request,
							ResponseMessage.END_RESULT);
			...
			case PREPARE_QUERY:
				...
				String instrumentedPlanAsXML = catalog.instrumentPlan(planID);
			...
			case SET_TUNABLE:
				...
				if (parts.length != 3) {
					sendErrMessage(request, ResponseMessage.ERROR,
							"Invalid syntax in SET_TUNABLE request");
					break;
				}
				// Get planID
				planID = parts[0];
				...
				try {
					...
					sendErrMessage(request, ResponseMessage.ERROR,
							"Could not find setter method for " + opID + "."
									+ tunable);
				} catch (SecurityException e) {
					sendErrMessage(request, ResponseMessage.ERROR,
							"Could not access setter method for " + opID + "."
									+ tunable);
				} catch (IllegalArgumentException e) {
					sendErrMessage(request, ResponseMessage.ERROR,
							"Problem invoking setter method for " + opID + "."
									+ tunable + ": " + e.getMessage());
	 ...
	 * @throws IOException
	 */
	private void fetchEpoch(RequestMessage request, ServerQueryInfo sqi)
			throws IOException {
	...
	private ServerQueryInfo processQPQuery(Plan plan, RequestMessage request)
		...
		boolean isSynchronous = (request.getRequestType() == RequestMessage.RequestType.SYNCHRONOUS_QP_QUERY);
	...
	public void killQuery(int queryID) {
		...
		assert queryInfo.getTransmitter() != null : "KT way bad transmitter is null";
	...
	/**
	 * Gracefully shutdow the cunnection to this client cleans up all the
	 * outstanding queryies
	 */
	...
	public void disconnectIntermittentConnection(RequestMessage request,
			int queryID) throws IOException {
		// ServerQueryInfo sqi = queryList.remove(queryID);
		// assert sqi != null;
		// send end of result msg;
		sendResponse(new ResponseMessage(request, ResponseMessage.END_RESULT));
	...
	/**
	 * Send the queryId that has been assigned to this query. This is the first
	 * things that is sent to the client after a query is received
	 * 
	 * @param request
	 *            The initial request
	 */
	private void sendQueryId(RequestMessage request) throws IOException {
		ResponseMessage resp = new ResponseMessage(request,
				ResponseMessage.SERVER_QUERY_ID);
		sendResponse(resp);
	}
		...
		for (long threadID : threadIDs) {
			...
			if (diff > 0)
				System.err.println("CPUTIME\t"
						+ mxbean.getThreadInfo(threadID).getThreadName() + "\t"
						+ diff);
		}
	}

	/**
	 * Class for storing the ServerQueryInfo objects into a hashtable and
	 * accessing it Essentially a wrapper around Hashtable class with similar
	 * functionality
	 */
	class QueryList {
		Hashtable queryList;
		 ...
		 * removedQueryList.addAll(queryList.values()); queryList.clear();
		 * 
		 * }
		 */
