...
public class CollectInstrumentationThread extends SourceThread {
	...
	@Tunable(name = "period", type = Tunable.TunableType.INTEGER, setter = "setPeriod", description = "Collection period")
		...
		public void run() {
			for (Instrumentable op : ops) {
				...
				for (int i = 0; i < instrumentationNames.size(); i++) {
					...
					tuple.appendAttribute(new StringAttr(instrumentationNames
							.get(i)));
					if (value instanceof Node) {
						...
						for (int j = 0; j < tupleLength; j++) {
							...
							attrNode
									.setAttribute("name", ts.getVariableName(j));
							...
							if (tupleAttr instanceof XMLAttr) {
								Node n = doc.importNode(((XMLAttr) tupleAttr)
										.getNodeValue(), true);
								attrNode.appendChild(n);
							}
							if (tupleAttr instanceof BaseAttr) {
								String attrValue = ((BaseAttr) t
										.getAttribute(j)).toASCII();
								attrNode.appendChild(doc
										.createTextNode(attrValue));
							} else {
								if (!(tupleAttr instanceof Node)) {
									throw new PEException(
											"Unexpected tuple attribute type");
								}
