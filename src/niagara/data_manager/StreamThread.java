...
import niagara.firehose.FirehoseClient;
...
/**
 * Niagara DataManager StreamThread - an input thread to take a simple stream of
 * XML documents from a file or a socket, for now will just implement the
 * file??? Documents in stream are not demarcated, uses a modified xerces parser
 * to parse each document one at a time and put each parsed document into the
 * output stream
 */
public class StreamThread extends SourceThread {
	...
	public void run() {
		...
		try {
			...
			if (parser instanceof SAXDOMParser && spec.isStream()) {
				...
				((SAXDOMParser) (parser)).setOutputStream(outputStream, spec
						.delay());
				InputSource inputSource = new InputSource(inputStream);
				parser.parse(inputSource);
				/*
				 * }else if(parser.supportsStreaming()) { // This must be a
				 * modified Xerces parser in streaming mode
				 * 
				 * // stream is done when inputStream.read() returns -1
				 * InputSource inputSource = new InputSource(inputStream);
				 * sourcecreated=true; while(true) { // IOException is thrown
				 * when done - handled below parser.parse(inputSource);
				 ...
				 * try { bufferedInput = new BufferedReader(new
				 * InputStreamReader(inputStream)); buffer = new MyArray(8192);
				 * 
				 * boolean keepgoing = true; boolean startOfDoc = false; char
				 * cbuf[] = new char[5];
				 * 
				 * bufferedInput.read(cbuf, 0, 5); if(cbuf[0] != '<' || cbuf[1]
				 * != '?' || cbuf[2] != 'x' || cbuf[3] != 'm' || cbuf[4] != 'l')
				 * throw new DMException("Invalidly formed stream");
				 * 
				 * int cBufOffset = 0; int cBufSize = 5;
				 * 
				 * while(true) { buffer.reset(); startOfDoc = false;
				 * 
				 * // at this point cbuf should always hold <?xml
				 * buffer.appendChars(cbuf, 5);
				 * 
				 * while(!startOfDoc) { // look for string <?xml - indicates
				 * start of NEXT document
				 * 
				 * // fill the remaining portion of cbuf and check // for start
				 * of document cBufSize = 5; int ret = bufferedInput.read(cbuf,
				 * cBufOffset, 5-cBufOffset); if(ret == -1) throw new
				 * EOSException(); if(ret != 5-cBufOffset) cBufSize = ret; // at
				 * end of file cBufOffset = 0; // reset for next round;
				 * 
				 * if(cbuf[0] == '<' && cbuf[1] == '?' && cbuf[2] == 'x' &&
				 * cbuf[3] == 'm' && cbuf[4] == 'l') { // found start of
				 * document startOfDoc = true; } else {
				 * buffer.appendChar(cbuf[0]); for(int i = 1; i<cBufSize; i++) {
				 * if(cbuf[i] != '<') { buffer.appendChar(cbuf[i]); } else {
				 * for(int j = 0; j<cBufSize-i; j++) cbuf[j] = cbuf[j+i];
				 * cBufOffset = 5-i; } } } }
				 * 
				 * // parse the buffer and put the resulting document // in the
				 * output stream parseAndSendBuffer(buffer); } }
				 * catch(EOSException eosE) { // parse final doc and put in
				 * output stream, then return parseAndSendBuffer(buffer); }
				 */
			}

		} catch (org.xml.sax.SAXException saxE) {
			// System.err.println("StreamThread::SAX exception parsing document. Message: "
			System.err.println("StreamThread::Message: " + saxE.getMessage());
			shutdown = true;
			message = "SAX Exception " + saxE.getMessage();
		} catch (java.lang.InterruptedException intE) {
			System.err
					.println("StreamThread::Interruped Exception::run. Message: "
							+ intE.getMessage());
			shutdown = true;
			...
			System.err.println("StreamThread::Unknown Host: host: " + "Message"
					+ unhE.getMessage());
			shutdown = true;
	...
	/**
	 * Indicate I'm going away and am not going to read any more data in the
	 * stream - to try to close up sockets, etc.
	 */
	private void closeInputStream() throws java.io.IOException {
		if (spec instanceof FileScanSpec) {
			closeFileStream();
		} else if (spec instanceof FirehoseSpec) {
			closeFirehoseStream();
	...
	private void closeFileStream() throws java.io.IOException {
		if (inputStream != null) {
			inputStream.close();
		}
	}

	private void closeFirehoseStream() throws java.io.IOException {
		if (firehoseClient != null) {
	...
	// new InputSource(new CharArrayReader(buffer.getBuf(), 0,
	// buffer.getLen()));
	// parser.parse(inputSource);
	// outputStream.put(parser.getDocument());
	// }
	...
	public boolean equals(Object o) {
		if (o == null || !(o instanceof StreamThread))
			return false;
	...
	public int hashCode() {
		...
		return hashCodeNullsAllowed(spec);
	}
