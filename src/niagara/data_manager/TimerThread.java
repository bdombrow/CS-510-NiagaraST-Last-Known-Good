...
public class TimerThread extends SourceThread {
	...
	public TimerThread(String relative, int period, int slack, int granularity,
			int delay, int warp, String name) {
	 ...
	 */
	public Op opCopy() {
		return new TimerThread(relative, period, slack, granularity, delay,
				warp, name);
	}
	...
	public boolean equals(Object o) {
		...
		return name.equals(other.name) && relative == other.relative
				&& period == other.period && slack == other.slack
				&& granularity == other.granularity && delay == other.delay
				&& warp == other.warp;
	}
	 ...
	 */
	public int hashCode() {
		return name.hashCode() ^ period ^ slack ^ granularity ^ delay ^ warp
				^ relative.hashCode();
	}
	...
	/**
	 * @see niagara.optimizer.colombia.PhysicalOp#findLocalCost(niagara.optimizer.colombia.ICatalog,
	 *      niagara.optimizer.colombia.LogicalProperty[])
	 */
	...
	class TimerThreadTask extends TimerTask {
			...
			if (tt.relative.equalsIgnoreCase("now"))
				...
				try {
					...
					throw new PEException(
							"Invalid date string passed to TimerThread!");
		...
		public void run() {
			...
			long currentTime = (System.currentTimeMillis() - offset) * tt.warp
					- tt.slack;
