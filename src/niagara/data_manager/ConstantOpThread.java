 ...
 * document and put it into the stream; based on FirehoseThread
 */
...
public class ConstantOpThread extends SourceThread {
	...
	public void run() {
		...
		try {
			...
			throw new PEException(
					"Error processing document in ConstantOpThread ");
	...
	/**
	 * process the incoming message which was just put in the message buffer -
	 * this means parse it and put the resulting Document in the output stream
	 */
	private void processDocument() throws SAXException, ShutdownException,
			IOException {
		try {
			...
			if (doc.getDocumentElement().getTagName().equals("stream")) {
				...
				NodeList tuples = doc.getDocumentElement()
				...
				// A regular costant doc.
				outputStream.put(doc);

		} catch (java.lang.InterruptedException e) {
			System.err
					.println("Thread interrupted in ConstantOpThread::processMessageBuffer");
		}
		return;
	}
	...
	public Cost findLocalCost(ICatalog catalog, LogicalProperty[] InputLogProp) {
		return new Cost(catalog.getDouble("document_parsing_cost") + 1
	...
	public boolean equals(Object o) {
		if (o == null || !(o instanceof ConstantOpThread))
			return false;
