...
public class Present extends BinaryOperator {
	...
	/*
	 * public void dumpChildrenInXML(StringBuffer sb) { sb.append(">");
	 * pred.toXML(sb); sb.append("</join>"); }
	 */
	...
	public void loadFromXML(Element e, LogicalProperty[] inputProperties,
		...
		if (punctAttrVals.length != 2) {
			throw new InvalidPlanException(
					"Present needs punctattr from each side of input");
		}

		punctAttrs = new Attrs[2];
		punctAttrs[0] = new Attrs(Variable.findVariable(inputProperties[0],
				punctAttrVals[0]));
		punctAttrs[1] = new Attrs(Variable.findVariable(inputProperties[1],
				punctAttrVals[1]));
	}
