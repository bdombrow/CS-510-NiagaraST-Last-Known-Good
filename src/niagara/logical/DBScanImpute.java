...
/**
 * Modified version of DBScan to support class example of imputation. This class
 * represents stream Scan operator that fetches data from a stream, parses it
 * and returns the DOM tree to the operator above it.
 */

public class DBScanImpute extends NullaryOperator {
	...
	public DBScanImpute(DBScanSpec dbScanSpec, Attribute[] variables,
	...
	public Op opCopy() {
		return new DBScanImpute((DBScanSpec) dbScanSpec, variables, sSpec,
				pfSpec);
	}

	public boolean equals(Object obj) {
		if (obj == null || !(obj instanceof DBScanImpute))
			return false;
		if (obj.getClass() != DBScanImpute.class)
			return obj.equals(this);
		DBScanImpute other = (DBScanImpute) obj;
		return dbScanSpec.equals(other.dbScanSpec)
				&& equalsNullsAllowed(pfSpec, other.pfSpec)
				&& variables.equals(other.variables)
				&& equalsNullsAllowed(sSpec, other.sSpec);
	}
	...
	public void loadFromXML(Element e, LogicalProperty[] inputProperties,
		...
		dbScanSpec = new DBScanSpec(qstring, attrNames, attrTypes, timeattr,
				type);
		...
		if (similarityStr != "") {
			...
			if (similarityMetric.length != 3 && similarityMetric.length != 4)
				throw new InvalidPlanException("Invalid similarity string "
						+ similarityStr);
			sSpec = new SimilaritySpec(similarityMetric[0], Integer
					.valueOf(similarityMetric[1]), Integer
					.valueOf(similarityMetric[2]), false);
		...
		if (prefetchStr != "") {
			...
			if (prefetch.length != 2)
				throw new InvalidPlanException("Invalid prefetch string "
						+ prefetchStr);
