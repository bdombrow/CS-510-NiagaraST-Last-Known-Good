...
abstract public class Comparison extends Predicate {
	...
	public static Comparison newComparison(int operator, Atom left, Atom right) {
		if (left.isVariable() && right.isConstant())
			return new VarToConstComparison(operator, (Variable) left,
					(Constant) right);
		if (left.isVariable() && right.isVariable())
			return new VarToVarComparison(operator, (Variable) left,
					(Variable) right);
		if (left.isConstant() && right.isVariable())
			return new ConstToVarComparison(operator, (Constant) left,
					(Variable) right);

		if (left.isPath() && right.isConstant())
			return new PathToConstComparison(operator, (Path) left,
					(Constant) right);
