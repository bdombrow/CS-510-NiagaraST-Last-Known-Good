...
abstract public class Predicate implements condition {
	...
	/**
	 * Split this predicate into a conjunction of two predicates: one that only
	 * references the specified variables, and another with no such restrictions
	 */
	...
	/**
	 * Split this predicate into a conjunction of two predicates: a conjunction
	 * of equality comparisons between attributes from leftAttrs and rightAttrs
	 * (that can be used in a hash-based join), and another with no such
	 * restrictions
	 */
	...
	public UpdateableEquiJoinPredicateList toEquiJoinPredicateList(
			Attrs leftAttrs, Attrs rightAttrs) {
		throw new PEException(
				"Cannot convert an arbitrary predicate to an equijoin predicate list");
	}
	...
	/**
	 * Rough selectivity estimate for this predicate. Heuristics solidly based
	 * on thin air!
	 */
	...
	public static Predicate loadFromXML(Element e,
			LogicalProperty[] inputProperties) throws InvalidPlanException {
		if (e == null)
			return True.getTrue();
		...
		if (nodeName.equals("false")) {
			...
			if (type == opType.UNDEF) {
				throw new InvalidPlanException("Unrecognized predicate op: "
						+ e.getAttribute("op"));
			}
	...
	private static Atom parseAtom(Element e, LogicalProperty[] inputProperties)
		...
		if (name.equals("number")) {
			...
			try {
				...
				throw new InvalidPlanException("Could not parse '" + val
						+ "' as a number.");
			}
			...
			try {
				...
				throw new InvalidPlanException(
						"Syntax error in regular expression: " + msg);
			...
			return new Path((Variable) Variable.findVariable(inputProperties, e
					.getAttribute("var")), path);
		} else { // variable
			...
			if (v != null)
				...
				throw new InvalidPlanException("Unknown variable name: "
						+ varname);
		}
	}
