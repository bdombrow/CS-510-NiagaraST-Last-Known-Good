...
import niagara.xmlql_parser.SchemaUnit;
import niagara.xmlql_parser.Util;
...
import niagara.xmlql_parser.inClause;
import niagara.xmlql_parser.opType;
import niagara.xmlql_parser.pattern;
import niagara.xmlql_parser.patternLeafNode;
import niagara.xmlql_parser.pnode;
...
import niagara.xmlql_parser.set;
...
/**
 * 
 * This class generates logical plan from a syntax tree for XML-QL. There is no
 * optimization. It uses simple hurestic to first generate a sequence of Scan
 * nodes for each element to scan and then add a Select node if the select
 * predicates are over the elements in one chain, merges two chains by a Join
 * node if there are some predicates with variables in both the chains. Finally
 * there are Construct nodes at the top.
 * 
 * 
 */
...
public class LogPlanGenerator {
	 ...
	 * 
	 * @param the
	 *            syntax tree for the XML-QL query
	 */
	...
	/**
	 * gives an equivalent regular expression for the tag variables. $a IN
	 * {author, editor} will give the following regular expression for the tag
	 * variable $a : | / \ author editor
	 * 
	 * @param the
	 *            variable
	 ...
	 * dtdScan as the leaf node for an IN clause. The IN clause <book> <author>
	 * </> </> IN book.xml will give the following chain: Scan book ^ Scan
	 * author ^ dtdScan book.xml
	 * 
	 * @param the
	 *            IN clause
	...
	private LogNode genLogPlan(inClause inc) {
			...
			if (curpat instanceof patternLeafNode) {
				...
				if (type == dataType.VAR) {
					...
					if (varList.lookUp(var) != null) {
						String prevVar = var;
						var = getTmpVar();
						listOfPredicates.addElement(Comparison.newComparison(
								opType.EQ, new Variable(var), new Variable(
										prevVar)));
					}
				} else {
					...
					// identifier
					StringConstant sc = new StringConstant((String) leafData
							.getValue());
					var = getTmpVar();
					listOfPredicates.addElement(Comparison.newComparison(
							opType.EQ, new Variable(var), sc));
				}
				schattr = new schemaAttribute(i + 1, var);
				varList.addVar(var, schattr);
			}
		...
		for (int i = 1; i < tupleDes.numAttr(); i++) {
			...
			if (i == 1)
				scan.setScan(new schemaAttribute(su.getBackPtr(),
						varType.ELEMENT_VAR, null), su.getRegExp());
			else
				scan.setScan(new schemaAttribute(su.getBackPtr(), null), su
						.getRegExp());
	...
	/**
	 * Generate a chain of Scan nodes for eact IN clause and add it to the list
	 * of partial trees
	 */
	private void initialLogTrees() {
		int numOfInClause = listOfInClause.size();
		for (int i = 0; i < numOfInClause; i++)
			partialTree.addElement(genLogPlan((inClause) listOfInClause
					.elementAt(i)));
	}

	/**
	 * Scan through the list of predicates and check if the variables present in
	 * them are contained in the variables encountered in any of the partial
	 * trees. If yes, then add a Select node with these predicates as the
	 * Selection condition.
	 */
	...
	private LogNode joinTree(LogNode leftTree, LogNode rightTree, Vector comVar) {
		...
		// XXX vpapad: leftJoinAttr and rightJoinAttr at this
		// point contain schemaAttributes - we'll convert them
		// to Variable ArrayLists for setJoin
		ArrayList left = new ArrayList();
		for (int i = 0; i < leftJoinAttr.size(); i++)
			left.add(new OldVariable((schemaAttribute) leftJoinAttr.get(i)));
		ArrayList right = new ArrayList();
		for (int i = 0; i < leftJoinAttr.size(); i++)
			left.add(new OldVariable((schemaAttribute) rightJoinAttr.get(i)));

		// creates a predicate for the equi-join
		// Predicate equiJoinPred = Util
		// .makePredicate(leftJoinAttr, rightJoinAttr);

		// ??? KT - VPAPAD is the setting of ext join right???
		join.setJoin(joinPredicate, left, right, Join.NONE);
	...
	/**
	 * From the list of partial trees, find the pairs that have some variables
	 * in common and then replace it with their join. This process is repeated
	 * until no more trees can be joined.
	 */
	...
	/**
	 * Try to join the partial trees whose union of variables contain all the
	 * variables of some predicates.
	 */
		...
		while (i < (partialTree.size() - 1)) {
			...
			while (j < partialTree.size()) {
				...
				if (preds.size() != 0) {
					...
					int leftRelSize = leftSchema.numAttr();
					newSchema = Util.mergeSchemas(leftSchema, rightSchema);
					newVarTbl = Util.mergeVarTbl(leftVarTbl, rightVarTbl,
							leftRelSize);
					join = new Join();
					join.setCartesian(joinPredicate);
					newTree = new LogNode(join, leftTree, rightTree);
					newTree.setVarTbl(newVarTbl);
					newTree.setSchema(newSchema);
	...
	/**
	 * If the number of partial trees is still more than one, after consuming
	 * all the predicates and none of them have variables in common with other,
	 * then they are joined without any join predicate (i.e. cartesian product)
	 * to get only one tree.
	 */
	...
	/**
	 * This is a recursive function that is used to flatten the n-ary tree
	 * representation of the pattern of the construct part of the query. Ecah
	 * root of the subtree of this n-ary tree has a corresponding schema
	 * attribute. An internal node is has the tag name and these schema
	 * attributes as its children. This function is passed the length of the
	 * current Schema, so that the position of the next schema attribute is one
	 * more than this.
	 * 
	 * @param the
	 *            construct node
	 * @param the
	 *            position or value of the first schema attribute
	 */
	private int addToList(constructBaseNode _cn, int initial_slot) {
		...
		for (int i = 0; i < numChild; i++) {
			childPtrs[i] = addToList((constructBaseNode) children.elementAt(i),
					first_slot);
		...
		for (int i = 0; i < numChild; i++) {
			...
			if (baseNode instanceof constructLeafNode)
				...
				newLeafNode = new constructLeafNode(new data(dataType.ATTR,
						new schemaAttribute(childPtrs[i], varType.ELEMENT_VAR,
								null)));
	...
	/**
	 * Use the addToList method to flatten the n-ary tree for the pattern and
	 * then for each construct node in the list generate a construct operator
	 * and add to the top of the tree
	 */
	private void addConstruct() {
		...
		int tupleSize, numOfConstruct;// numchild, pos, numOfConstruct;
		// schemaAttribute sa;
		...
		if (partialTree.size() != 1) {
			System.out
					.println("can't have more than one tree before adding construct operator");
			return;
		}
		...
		for (int i = 0; i < numOfConstruct; i++) {
			constructTagPart = (constructInternalNode) listOfConstruct
					.elementAt(i);
	...
	public static void main(String argv[]) {
		...
		try {
			...
			Scanner s = new Scanner(new EscapedUnicodeReader(new FileReader(
					argv[0])));
