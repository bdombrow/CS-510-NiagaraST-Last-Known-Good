...
/** The <code>Magic</code> operator sends feedback punctuation on time intermittently
 * 
 *  To do:
 *  (1) Clean up, since the operator is inherited from select and does not need all methods (careful, I broke something on first try)
 *  (2) As with all FP work, must actually send punctuation instead of a flattened message.
 *  */
...
public class Magic extends UnaryOperator {
    ...
    public Magic(Predicate pred, long interval, Attribute pAttr, Attribute tAttr, boolean propagate){
	this.pred = pred;
    ...
    public Magic(Magic selOp) {
        this(selOp.pred, selOp.interval, selOp.pAttr, selOp.tAttr, selOp.propagate);
    }
    ...
    public String toString() {
        return " magic punctattr = " + pAttr.getName() + "timeattr = " + tAttr.getName() + " interval = " + interval + pred.toString() + " propagate = " + propagate;
    }
    ...
    public void dumpChildrenInXML(StringBuffer sb) {
        sb.append(">");
        sb.append("punctattr = " + pAttr + " timeattr = " + tAttr + " interval = " + interval + " propagate = " + propagate);
        pred.toXML(sb);
        sb.append("</magic>");
    }
    ...
    public void loadFromXML(Element e, LogicalProperty[] inputProperties, Catalog catalog)
		...
		if (tAttrStr.length() == 0)
			throw new InvalidPlanException("Bad value for 'timeattr' for : " + id);
