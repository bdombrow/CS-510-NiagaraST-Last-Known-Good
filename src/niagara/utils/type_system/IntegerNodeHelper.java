...
/**
 * Class <code> IntegerNodeHelper </code> which performs functions on two nodes,
 * interpreting the values as ints.
 * 
 * Intended to add a "int type" to the niagara system.
 * 
 * Class <code> IntegerNodeHelper </code> implements numeric functions on nodes
 * whose content is interpreted as integers
 * 
 * 
 * @version 1.0
 * 
...
public class IntegerNodeHelper extends NumberNodeHelperBase implements
		NumberNodeHelper {
	...
	public boolean nodeEquals(Node lNode, Node rNode) {
		/*
		 * should I do some optimization here and try to store a local value in
		 * the Nodes?? Double lVal = (Double)lNode.getLocalValue(); Double rVal
		 * = (Double)rNode.getLocalValue(); if(lVal != null && rVal != null) {
	...
	public boolean lessThan(Node lNode, Node rNode) {
		return ((Integer) valueOf(lNode)).intValue() < ((Integer) valueOf(rNode))
				.intValue();
	}

	/**
	 * Averages the content of two nodes - content is interpreted as integers
	 * 
	 * @param lNode
	 *            First node input to average
	 * @param rNode
	 *            Second node input to average
	 * @param resultNode
	 *            Place to put the result of the average calculation
	 * 
