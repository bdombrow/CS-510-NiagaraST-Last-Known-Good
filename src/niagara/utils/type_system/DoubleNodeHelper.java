...
/**
 * Class <code> DoubleNodeHelper </code> which performs functions on two nodes,
 * interpreting the values as double.
 * 
...
public class DoubleNodeHelper extends NumberNodeHelperBase implements
		NumberNodeHelper {
	...
	public boolean lessThan(Node lNode, Node rNode) {
		return ((Double) valueOf(lNode)).doubleValue() < ((Double) valueOf(rNode))
				.doubleValue();
	}
	...
	public boolean average(Node lNode, Node rNode, Node resultNode) {
		...
		if (rCnt != 1) {
			System.out
					.println("KT throwing PEException - HOPE IT IS HANDLED RIGHT");
